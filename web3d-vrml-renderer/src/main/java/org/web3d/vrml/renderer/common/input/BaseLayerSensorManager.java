...
public abstract class BaseLayerSensorManager implements LayerSensorManager {
    ...
    protected Set<VRMLNodeType> allSensorNodes;

    /** The collection of view listeners */
    //protected HashSet<ViewListener> viewListeners;

    /** The collection of origin listeners */
    protected Set<OriginListener> originListeners;
    ...
    public BaseLayerSensorManager() {
        ...
        allSensorNodes = new HashSet<VRMLNodeType>();
        originListeners = new HashSet<OriginListener>();
    }
    ...
    public void setOriginManager(OriginManager manager) {
		...
		inputHandler.setOriginManager(manager);
    ...
    public void addSensors(NodeArray sensors) {
		...
		doubleCheckLayerId(node, true);
            ...
            if(node instanceof OriginListener) {
                    originListeners.add((OriginListener)node);
            }
    ...
    public void removeSensors(NodeArray sensors) {
	    ...
	    if(node instanceof OriginListener)
		originListeners.remove((OriginListener)node);
...
*/
        ...
        nodes =
            scene.getBySecondaryType(TypeConstants.OriginManagedNodeType);
        processAddedSensors(nodes);
     ...
     * @param nodes The list of nodes to check
     */
    private void processAddedPointingDeviceSensors(List nodes) {
        int size = nodes.size();
     ...
     * @param nodes The list of nodes to check
     */
    private void processAddedSensors(List nodes) {
        int size = nodes.size();
     ...
     * @param nodes The list of nodes to check
     */
    private void processRemovedSensors(List nodes) {
        int size = nodes.size();
     ...
     * @param nodes The list of nodes to check
     */
    private void processRemovePointingDeviceSensors(List nodes) {
        int size = nodes.size();
    ...
    /**
     * Temporary method - some nodes (GeoElevationGrids) do not get a layer ID
     * set on them. this adds/removes them to/from the list under certain conditions.
     */
    private void doubleCheckLayerId(VRMLNodeType node, boolean add) {
        int[] layerIds = node.getLayerIds();

        if ((layerIds == null) && (layerId == 0) && (node instanceof OriginListener)) {
            if (add) {
                originListeners.add((OriginListener)node);
                allSensorNodes.add(node);
            } else {
