 ...
 */
public abstract class BaseFog extends BaseBindableNode
    ...
    protected static final int FIELD_COLOR = LAST_BINDABLE_INDEX + 1;

    /** Index for the fogType field */
    protected static final int FIELD_FOGTYPE = LAST_BINDABLE_INDEX + 2;

    /** Index for the visibilityRange field */
    protected static final int FIELD_VISIBILITY_RANGE = LAST_BINDABLE_INDEX + 3;
    ...
    static {
        ...
        idx = new Integer(FIELD_FOGTYPE);
        fieldMap.put("fogType", idx);
        fieldMap.put("set_fogType", idx);
        fieldMap.put("intensity_fogType", idx);
     ...
     */
    protected BaseFog(VRMLNodeType node) {
        this();
    ...
    //----------------------------------------------------------
    // Methods required by the VRMLFogNodeType interface.
    //----------------------------------------------------------
    ...
    public void setColor(float[] color)
        ...
        FieldValidator.checkColorVector("Fog.color",color);
        vfColor[0] = color[0];
        vfColor[1] = color[1];
        vfColor[2] = color[2];
    ...
    //----------------------------------------------------------
    // Methods required by the NRVRMLNodeType interface.
    //----------------------------------------------------------
    ...
    public void sendRoute(double time,
        ...
        try {
            ...
            System.err.println("Fog sendRoute: No field!" + ife.getFieldName());
        } catch(InvalidFieldValueException ifve) {
            System.err.println("Fog sendRoute: Invalid field Value: " +
                               ifve.getMessage());
        }
    }
    ...
    protected void setFogType(String type) throws InvalidFieldValueException {
        if(type.equals(LINEAR_TYPE) || type.equals(EXPONENTIAL_TYPE)) {
            vfFogType = type;
            Integer i_type = typeMap.get(type);
