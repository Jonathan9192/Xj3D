...
public class OGLIndexedTriangleStripSet extends BaseIndexedTriangleStripSet
    ...
    protected void buildImpl() {
        ...
        if (vfNormal != null) {
            ...
            if(((changeFlags & NORMALS_CHANGED) != 0)) {
                if(!vfNormalPerVertex) {
                    errorReporter.warningReport("Normal per face not supported in IndexedTriangleStripSet", null);
                } else
                    updateNormalArray();
            }
            ...
            if (indexes == null || (changeFlags & COORDS_INDEX_CHANGED) != 0) {
                ...
                // calculate the number of triangles, used to size the index array
                int num_strip_vertex = 0;
                int num_tri = 0;
                for(int i = 0; i < numIndex; i++) {
                    if(vfIndex[i] == -1) {
                        num_tri += num_strip_vertex - 2;
                        num_strip_vertex = 0;
                    } else {
                        num_strip_vertex++;
                    }
                ...
                if (vfIndex[numIndex-1] != -1) {
                    // the last strip may or may not be terminated with a -1,
                    // if not - update the count for the last strip
                    num_tri += num_strip_vertex - 2;
                }

                // Allocate array for 3 vertices plus a -1 per triangle
                int size = num_tri * 4;
        ...
        if (newImpl) {
            ...
            if((changeFlags & COORDS_CHANGED) != 0 || (changeFlags & COORDS_INDEX_CHANGED) != 0) {
                coordChanged = true;
                if (implGeom.isLive())
                    implGeom.boundsChanged(this);
                else
