 ...
 */
class OGLMainSceneBuilder extends CRMainSceneBuilder {
     ...
     * be producing nodes that conform to the {@link OGLVRMLNode} interface
     ...
     */
    OGLMainSceneBuilder(VRMLNodeFactory fac) {
        super(fac);
    }

    /**
     * Notification of the start of a node. This is the opening statement of a
     * node and its DEF name. USE declarations are handled in a separate
     * method.
     *
    ...
    public void startNode(String name, String defName)
        ...
        if(protoMap.containsKey(name) || externProtoMap.containsKey(name)) {
            ...
            if(proto_def == null) {
                ...
                if (proto_def == null) {
                    ...
                    ex_proto.addInstance(nodeStack[topOfStack],
                        currentFieldIndex, node);
                }
            }

            if(proto_def != null) {
                if(protoCreator == null) {
                    protoCreator = new OGLProtoCreator(nodeFactory,
                                                       worldURL,
                                                       majorVersion,
                                                       minorVersion);
                ...
                node = (OGLVRMLNode)protoCreator.newInstance(proto_def,
                                                             root,
                                                             majorVersion,
                                                             minorVersion,
                                                             inStatic);
            ...
            node = (OGLVRMLNode)nodeFactory.createVRMLNode(name,
                                                           inStatic);
        }

        if (node == null) {
            errorReporter.errorReport("Unknown node: " + name, null);
        }
