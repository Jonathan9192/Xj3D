...
public abstract class BaseSequencerNode extends AbstractNode
     ...
     */
    public void setNext() {
        int key = currentIndex + 1;

        if(key >= numKey)
            key = 0;

        float fraction = vfKey[key];

        if(key < numKey - 1)
            fraction = (vfKey[key] + vfKey[key + 1]) * 0.5f;

        setFraction(fraction);

        // Reset this after set fraction because setFraction calls
        // setCurrentKey, which will overwrite this value
        currentIndex = key;
    }
    ...
    protected void setCurrentKey() {
        ...
        if((idx == 0) && (vfFraction > vfKey[numKey - 1]))
            idx = numKey - 1;
