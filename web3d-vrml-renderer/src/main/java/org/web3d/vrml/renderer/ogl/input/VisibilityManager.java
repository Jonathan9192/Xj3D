...
class VisibilityManager {
    ...
    private HashSet<OGLVisibilityListener> activeVisSensors;

    /** The newly visible sensors this frame. */
    private HashSet<OGLVisibilityListener> newVisSensors;
    ...
    /**
     * Construct a new manager for visibility sensors.
     */
    VisibilityManager() {
        ...
        activeVisSensors = new HashSet<OGLVisibilityListener>();
        newVisSensors = new HashSet<OGLVisibilityListener>();
    ...
    /**
     * Set the container for the view environment data. Used to process the view
     * frustum when doing visibility sensor handling.
     *
    ...
    /**
     * Clear the current list of items internally. This is because the world has
     * changed and that the currently cached data is no longer usable.
     */
    void clear() {
    ...
    /**
     * Process this frame using the given position and orientation of the user
     * position in Virtual world coordinates.
     *
    ...
    void processFrame(Point3f pos, AxisAngle4f orient, Matrix4f vpMatrix) {
        ...
        List<SceneGraphPath> pick_list = (List<SceneGraphPath>) picker.foundPaths;
        if (pick_list != null) {
            pick_list.clear();
        }
        ...
        if (picker.pickCount != 0) {
            ...
            newVisSensors.clear();
            //ArrayList pick_list = (ArrayList)picker.foundPaths;
            pick_list = (List<SceneGraphPath>) picker.foundPaths;

            for (int i = 0; i < picker.pickCount; i++) {
                SceneGraphPath found = pick_list.get(i);
        ...
        if (size != 0) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                OGLVisibilityListener l = list[i];
        ...
        // Swap over lists
        HashSet<OGLVisibilityListener> tmp = activeVisSensors;
     ...
     */
    private void resizeList(int maxSize) {
