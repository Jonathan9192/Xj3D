...
public abstract class BaseScript extends AbstractDynamicFieldNode
    ...
    private static final HashSet<String> validVrml97Types;
    ...
    private static final HashSet<String> forbiddenFields;
     ...
     */
    static {
        validVrml97Types = new HashSet<String>();
        ...
        forbiddenFields = new HashSet<String>();
...
System.out.println("Script needs to handle notifyExternProtoLoaded");
     ...
     *
     * @param timestamp The VRML time that the initialisation occurred at
     */
    public void initialize(double timestamp) {
        try {
            ...
            // Use this test instead of instanceof to avoid everyone having to install Rhino
            if (className.equals("org.mozilla.javascript.EcmaError")) {
                String msg;

                msg = "Script failed to initialize: \n";
                BufferedReader rdr = new BufferedReader(new StringReader(vfUrl[0]));
                String st;
                int line = 0;
                ...
                InvalidFieldFormatException iffe = new InvalidFieldFormatException(e.toString());
                errorReporter.errorReport(msg,iffe);
            } else {
                errorReporter.errorReport("Script failed to initialize2: " + vfUrl[0], e);
            }
        }
    }
    ...
    public void setValue(int index, VRMLNodeType[] children, int numValid)
        ...
        if(inSetup) {
            ...
            if(cur_node == null) {
                VRMLNodeType[] copy = new VRMLNodeType[numValid];
                System.arraycopy(children, 0, copy, 0, numValid);
                fieldValueMap.put(index, copy);
            } else if(cur_node instanceof VRMLNodeType) {
     ...
     */
    @Override
    public final int appendField(VRMLFieldDeclaration field)
...
// defined, even if it is a default.
     ...
     * EAI fulfills its requirements, but may be useful elsewhere.
     *
     * @param index The index of destination field to set
     ...
     */
    protected final void copy(BaseScript node) {

        List<VRMLFieldDeclaration> fields = node.getAllFields();
        int size = fields.size();

        for(int i = 0; i < size; i++) {
            VRMLFieldDeclaration decl = fields.get(i);
            ...
            try {
                ...
                System.err.println("Error copying field in abstract node");
                fe.printStackTrace();
            }
        }
    }
