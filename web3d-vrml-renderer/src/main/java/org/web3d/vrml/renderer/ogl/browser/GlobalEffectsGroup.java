...
public class GlobalEffectsGroup extends Group
    ...
    // TODO: Not sure why this need to be closer.  Tearing happens otherwise.  A change to
...
//    private static final float BOX_SIZE = 0.7071f;
    ...
    private Map<TextureUnit, Texture2D> texUnitToTexture;
    ...
    public GlobalEffectsGroup(SimpleScene scene) {
        ...
        texUnitToTexture = new HashMap<TextureUnit, Texture2D>();
    ...
    public void updateNodeBoundsChanges(Object src) {
        if(src == cameraTransform)
			...
			geomBackground.setShape(new Shape3D(), 1);
			geomBackground.setShape(new Shape3D(), 0);
		}
    }
    ...
    public void updateNodeDataChanges(Object src) {
        if(src == linearFog) {
            ...
            Texture tex = texUnitToTexture.remove((TextureUnit)src);
	...
	public void shutdown() {
		...
		if (geomBackground.isLive()) {
			geomBackground.boundsChanged(this);
		} else {
			updateNodeBoundsChanges(geomBackground);
		}
	}
...
//        tex_attr.setPerspectiveCorrectionMode(TextureAttributes.FASTEST);
        ...
        ba.setSourceBlendFactor(BlendAttributes.BLEND_SRC_ALPHA);
        ba.setDestinationBlendFactor(BlendAttributes.BLEND_ONE_MINUS_SRC_ALPHA);
