...
public class NavigationProcessor implements NodeUpdateListener, OGLTransformNodeType {
    ...
    /**
     * Fixed vector positive X axis
     */
    private static final Vector3f X_POS = new Vector3f(1, 0, 0);
    ...
    /**
     * The current navigation state either set from us or externally as the
     * mouse if being dragged around. This is different to the state that a
     * given mouse button will generate
     */
    private int navigationState;

    /**
     * Flag indicating that we are currently doing something and should ignore
     * any current mouse presses.
     */
    ...
    /**
     * Working variable that is used to track the duration it took to render the
     * last frame so we can compensate for this frame.
     */
     ...
     */
    private boolean useOriginManager;

    /**
     * The origin in use
     */
    private double[] local_origin;

    /**
     * Create a new mouse handler with no view information set. This handler
     * will not do anything until the view transform references have been set
     * and the navigation modes for at least one mouse button.
     */
    public NavigationProcessor() {
        ...
        data.owner = NavigationProcessor.this;
    ...
    /**
     * Set the center of rotation explicitly to this place. Coordinates must be
     * in the coordinate space of the current view transform group. The provided
     * array must be of least length 3. Center of rotation is used in examine
     * mode.
     *
     * @param center The new center to use
     * @param lookPos The position to look from. NULL is the current user
     * position.
     */
    ...
    /**
     * Set the view and it's related transform group to use and the path to get
     * there from the root of the scene. The transform group must allow for
     * reading the local to Vworld coordinates so that we can accurately
     * implement terrain following. A null value for the path is permitted.
     *
     * @param vp The current viewpoint
     * @param tg The transform just about the viewpoint used to move it around
     * in response to the UI device input
     ...
     * the same as the transform group presented in the view information.
     *
    ...
    /**
     * Set the ability to use a given state within the handler for a specific
     * mouse button (up to 3). This allows the caller to control exactly what
     * states are allowed to be used and with which buttons. Note that it is
     * quite legal to set all three buttons to the same navigation state
     *
    ...
    /**
     * Set the listener for collision notifications. By setting a value of null
     * it will clear the currently set instance
     *
     ...
     * same time although the code takes no steps to enforce this. If you do try
     * to do manual updates while also having the automated system, the results
     * are undefined.
     */
    ...
    public void startMove() {
        ...
        switch (navigationState) {
            ...
            case Xj3DNavigationState.FLY_LEVEL_STATE:
                allowCollisions = (collidables != null);
                allowTerrainFollowing = false;
                break;
    ...
    /**
     * Halt the current movement being processed. Any move input after this will
     * be ignored and the user will stop moving.
     */
     ...
     * gives a proportion of the set speed value to move in that direction. The
     * interpretation of the 3 components of the vector are described in the
    ...
    //----------------------------------------------------------
    /**
     * Get the transform matrix for this node. A reference is ok as the users of
     * this method will not modify the matrix.
     *
     * @return The matrix.
     */
    ...
    public void updateNodeBoundsChanges(Object src) {
        ...
        if (src == viewTg) {
            if (currentViewpoint != null) {
                // rem: retrieve the matrix set to the viewpoint, in case the
                // origin has been recalculated during this frame.....
                Matrix4f mtx = ((OGLTransformNodeType) currentViewpoint).getTransform();
                viewTx.set(mtx);
    ...
    public void originChanged() {
        if (local_origin != null) {
            centerOfRotation.x += local_origin[0];
            centerOfRotation.y += local_origin[1];
            centerOfRotation.z += local_origin[2];
        }
    ...
    /**
     * Set the world scale applied. This will scale down navinfo parameters to
     * fit into the world.
     *
     ...
     */
    public void setOriginManager(OriginManager manager) {
    ...
    private void processClockTick() {
        ...
        switch (navigationState) {
            ...
            case Xj3DNavigationState.FLY_LEVEL_STATE:
                processFlyLevelMotion();
                break;

            case NavigationState.LOOKAT_STATE:
                break;
    ...
    /**
     * Motion behaviour processing for the inspect mode (formerly the examine
     * mode).
     */
    ...
    /**
     * Motion behaviour processing for a fly mode that respects a gravitational
     * vector (+Y axis).
     */
    private void processFlyLevelMotion() {
        ////////////////////////////////////////////
        // rem: why is there a ceiling on this ?
        ...
        if (inputRotationY != 0) {
            ...
            float pitch_angle = upVector.angle(Y_UP);
            if (pitch_angle != 0) {
                // remove the pitch before applying the yaw
                ...
                // find which direction the pitch angle is in
                tmpVector.cross(upVector, Y_UP);
                float dp = tmpVector.dot(rightVector);
                if (dp > 0) {
                    pitch_angle = -pitch_angle;
                }
                hrzRot.set(X_POS, pitch_angle);

                tmpMatrix.setIdentity();
                tmpMatrix.setRotation(hrzRot);
                tmpMatrix.invert();

                viewTx.mul(tmpMatrix);
            }
            // apply the yaw to the view matrix
            float yaw_inc = inputRotationY * motionDelay;
            vrtRot.set(Y_UP, yaw_inc);

            tmpMatrix.setIdentity();
            tmpMatrix.setRotation(vrtRot);

            viewTx.mul(tmpMatrix);

            if ((pitch_angle != 0) || (inputRotationX != 0)) {
                // reincorporate the pitch to the view matrix
                float pitch_inc = inputRotationX * motionDelay;
                pitch_angle += pitch_inc;
                hrzRot.set(X_POS, pitch_angle);
            ...
            rightVector.normalize();
            float pitch_inc = inputRotationX * motionDelay;
            //hrzRot.set(rightVector, pitch_inc);
            hrzRot.set(X_POS, pitch_inc);
    ...
    private void processDefaultMotion() {
        ...
        // collisions.
        ////////////////////////////
        // rem: add the translation to the current, rather than
        // accumulating from the start...
        viewTx.get(viewTranslation);
        ////////////////////////////
        viewTranslation.add(oneFrameTranslation);
        viewTx.setTranslation(viewTranslation);
     ...
     *
     * @return true if the terrain following has successfully been applied false
     * means a collision.
     */
    private boolean checkTerrainFollowing() {
        ...
        terrainPicker.origin[0] = locationVector.x + oneFrameTranslation.x;
        terrainPicker.origin[1] = locationVector.y + oneFrameTranslation.y;
        terrainPicker.origin[2] = locationVector.z + oneFrameTranslation.z;

        terrainPicker.destination[0] = downVector.x;
        terrainPicker.destination[1] = downVector.y;
        terrainPicker.destination[2] = downVector.z;
        ...
        for (int i = 0; i < terrainPicker.pickCount; i++) {
            ...
            if (hds != null) {
                ...
                intersectionPoint.z = hds.getHeight(locationVector.x, locationVector.y);
            } else {
     ...
     * would be traveled next frame. If nothing is picked then no collision will
     * occur. If it does find something then obviously a collision will occur so
     * you do return a flag to say so.
     *
     * @param prefetch True if viewpoint info has already been fetched for this
     * frame
    ...
    private boolean checkCollisions() {
        ...
        switch (avatarRep) {
            case AVATAR_POINT:
                ...
                collidePicker.pickGeometryType = PickRequest.PICK_LINE_SEGMENT;
                collidePicker.origin[0] = locationEndPoint.x;
                collidePicker.origin[1] = locationEndPoint.y;
                collidePicker.origin[2] = locationEndPoint.z;

                collidePicker.destination[0] = locationPoint.x;
                collidePicker.destination[1] = locationPoint.y;
                collidePicker.destination[2] = locationPoint.z;
            ...
            case AVATAR_CYLINDER:
                ...
                collidePicker.destination[0] = kneePoint.x;
                collidePicker.destination[1] = kneePoint.y;
                collidePicker.destination[2] = kneePoint.z;
        ...
        float length = collisionVector.length();
    ...
    /**
     * Check the terrain height at the current position. This is done when we
     * first start moving a viewpoint with a mouse press.
     *
     * @return true if the terrain following has successfully been applied false
     * means a collision.
     */
    private void setInitialTerrainHeight() {
        ...
        terrainPicker.origin[0] = (locationVector.x);
        terrainPicker.origin[1] = (locationVector.y);
        terrainPicker.origin[2] = (locationVector.z);
    ...
    /**
     * Convenience method to walk the viewPath and calculate the root to virtual
     * world coordinate location of the viewpoint.
     */
