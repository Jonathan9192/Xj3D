...
public class OGLGeoLOD extends BaseGeoLOD implements
    ...
    /**
     * Scratch var for transferring center(translation) values
     */
    ...
    /**
     * Was the root group changed, false if it was the children. Used from
     * setContent and externproto loading.
     */
     ...
     */
    protected List<VRMLExternalNodeType> childExternalList;
     ...
     * node. If the node is not a group node, an exception will be thrown. It
     * does not copy the children nodes, just this node.
     *
    ...
    public void allEventsComplete() {
        ...
        if (levelChanged) {
            ...
            for (int i = 0; i < childExternalList.size(); i++) {
                VRMLSingleExternalNodeType node = (VRMLSingleExternalNodeType) childExternalList.get(i);
    ...
    public void setupFinished() {
        ...
        oglChildMap = new HashMap<VRMLNodeType, Node>();
    ...
    //----------------------------------------------------------
    /**
     * Set the content of this node to the given object. The object is then cast
     * by the internal representation to the form it needs. This should be one
     * of the forms that the preferred class type call generates.
     *
     ...
     * @param localPosition The vworld transform object for the class that
     * implemented this listener
     */
    public void visibilityStateChange(boolean visible,
            Point3f position,
            AxisAngle4f orientation,
            Matrix4f localPosition) {
//System.out.println("visibilityStateChange: "+ position +", "+ visible);
        /*
    ...
    /**
     * Notification that the object is still visible, but that the viewer
     * reference point has changed. Ignored for this implementation.
     *
    ...
    private void init() {
        ...
        childExternalList = new ArrayList<VRMLExternalNodeType>();
    }
    ...
    /**
     * Get references to any VRMLSingleExternalNodeTypes from the argument
     * VRMLScene.
     */
    private void getExternalNodeTypes(VRMLScene scene) {
        childExternalList.addAll((List<VRMLExternalNodeType>)
