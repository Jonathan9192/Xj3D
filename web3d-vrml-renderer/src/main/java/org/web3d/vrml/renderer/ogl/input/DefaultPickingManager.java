...
public class DefaultPickingManager
    ...
    private Map<String, Integer> assignedBitmasks;

    /** A count for the number of users of a particular mask */
    private Map<String, Integer> bitmaskCount;
    ...
    private HashSet<OGLPickingSensorNodeType> activeSensors;
    ...
    private List<Object> selectionArray;
    ...
    public DefaultPickingManager() {
        ...
        activeSensors = new HashSet<OGLPickingSensorNodeType>();
        selectionArray = new ArrayList<Object>();
        ...
        assignedBitmasks = new HashMap<String, Integer>();
        bitmaskCount = new HashMap<String, Integer>();
    ...
    public void processPickSensors(double time) {
        ...
        for(int i = 0; i < size; i++) {
            ...
            Map<PickableObject, VRMLNode> targets = sensor.getTargetMapping();
    ...
    public int addObjectType(String type) {
        ...
        Integer mask = assignedBitmasks.get(type);
        if(mask != null) {
            Integer count = bitmaskCount.get(type);
    ...
    public void removeObjectType(String type) {
        ...
        if(count.intValue() == 1) {
            ...
            Integer mask = assignedBitmasks.remove(type);
    ...
    private void checkIntersections(OGLPickingSensorNodeType sensor,
                                    Map<PickableObject, VRMLNode> targetMap) {
    ...
    private void processPointPicking(int intersectType,
                                     ...
                                     Map<? super PickableObject, ? super VRMLNode> targetMap) {
        ...
        for(int i = 0; !search_complete && i < num_index; i++) {
            for(int j = 1; !search_complete && j < index_list[i]; j++) {
                ...
                for(int k = 0; !search_complete && k < targets.length; k++) {
                    ...
                    if(singlePicker.foundPaths instanceof List)
                        potentials = (List)singlePicker.foundPaths;
                    else {
                    ...
                    if(intersectType != VRMLPickingSensorNodeType.INTERSECT_BOUNDS) {
                        ...
                        for(int l = 0; l < singlePicker.pickCount; l++) {
                            ...
                            for(int m = 0; !terminate && m < len; m++) {
                                PickableObject node = (PickableObject) path.getNode(m);
                                VRMLNode n = targetMap.get(node);
                ...
                if(intersectType != VRMLPickingSensorNodeType.INTERSECT_BOUNDS) {
                    ...
                    for(int j = 0; j < size; j++) {
                        ...
                        for(int k = 0; !terminate && k < len; k++) {
                            PickableObject node = (PickableObject) path.getNode(k);
                            VRMLNode n = targetMap.get(node);
