 ...
 */
public abstract class BaseOrientationInterpolator extends BaseInterpolatorNode {
     ...
     * locally.
     */
    protected BaseOrientationInterpolator() {
        super("OrientationInterpolator");

        hasChanged = new boolean[NUM_FIELDS];

        vfKeyValue = FieldConstants.EMPTY_MFROTATION;
        vfValue = new float[] {0, 0, 1, 0};
    }
     ...
     */
    protected BaseOrientationInterpolator(VRMLNodeType node) {
        this();
        ...
        try {
            ...
            if(field.numElements != 0) {
                vfKeyValue = new float[field.numElements * 4];
                System.arraycopy(field.floatArrayValue,
                                 0,
                                 vfKeyValue,
                                 0,
                                 field.numElements * 4);
                numKeyValue = field.numElements * 4;
            }
    ...
    //----------------------------------------------------------
    // Methods defined by BaseVRMLNodeTypeType
    //----------------------------------------------------------
    ...
    public VRMLFieldData getFieldValue(int index) throws InvalidFieldException {
        ...
        switch(index) {
            case FIELD_KEY_VALUE:
                ...
                fieldData.numElements = numKeyValue / 4;
                break;
    ...
    public void sendRoute(double time,
        ...
        try {
            switch(srcIndex) {
                ...
                case FIELD_VALUE:
                    destNode.setValue(destIndex, vfValue, 4);
                    break;
    ...
    /**
     * Set the key value (MFVec2f) to the new value. If the value is null, it
    ...
    /**
     * Set the value (SFRotation) to the new value. Null values will be
     * ignored.
     *
    ...
    private void rebuildInterpolator() {
        ...
        int size = (numKey < numKeyValue / 4) ? numKey : numKeyValue / 4;

        interpolator =
            new RotationInterpolator(size, RotationInterpolator.QUATERNION);
