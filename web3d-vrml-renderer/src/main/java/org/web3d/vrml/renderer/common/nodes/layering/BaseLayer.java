...
public class BaseLayer extends AbstractNode
    ...
    protected List<LayerListener> layerListeners;
    ...
    static {
        ...
        fieldMap.put("viewport",FIELD_VIEWPORT);
        fieldMap.put("addChildren",FIELD_ADDCHILDREN);
        fieldMap.put("removeChildren",FIELD_REMOVECHILDREN);
    }
    ...
    protected BaseLayer() {
        ...
        vfIsPickable = true;
        vfChildren = new ArrayList<VRMLNodeType>();
        layerId = -1;

        layerListeners = new ArrayList<LayerListener>(1);
    ...
    public void setValue(int index, VRMLNodeType child)
        ...
        switch(index) {
            ...
            case FIELD_ADDCHILDREN:
                if(inSetup)
                    throwInputOnlyWriteException("addChildren");
            ...
            case FIELD_VIEWPORT:
                if(!inSetup)
                    throwInitOnlyWriteException("viewport");
            ...
            case FIELD_REMOVECHILDREN:
                if(inSetup)
                    throwInputOnlyWriteException("removeChildren");
    ...
    protected void clearChildren() {
        ...
        for(int i = 0; i < num_kids; i++) {
            if(nodeTmp[i] instanceof VRMLBoundedNodeType)
                ((VRMLBoundedNodeType)nodeTmp[i]).setShared(false);
    ...
    protected void addChildNode(VRMLNodeType node)
        ...
        if(node instanceof VRMLBoundedNodeType) {
            ...
            if(impl instanceof VRMLBoundedNodeType)
    ...
    protected void removeChildNode(VRMLNodeType node) {
        if(node instanceof VRMLBoundedNodeType) {
            ...
            if(impl instanceof VRMLBoundedNodeType)
                ((VRMLBoundedNodeType)impl).setShared(false);
        }
