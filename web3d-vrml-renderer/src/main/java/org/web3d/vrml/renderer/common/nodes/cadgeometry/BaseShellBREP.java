 ...
 */
public abstract class BaseShellBREP extends AbstractNode implements VRMLBREPNodeType {

    /** Field Index for vertices */
    protected static final int FIELD_VERTEX = LAST_NODE_INDEX + 1;

    /** Field Index for vertices */
    protected static final int FIELD_FACE = LAST_NODE_INDEX +2 ;

    /** The last index in this node */
    protected static final int LAST_INDEX = FIELD_FACE;
    ...
    /** List of per-vertex attribute nodes provided */
    protected List<VRMLNodeType> vfFace;
    ...
    static {
        ...
        fieldDecl[FIELD_VERTEX] = new VRMLFieldDeclaration(
                FieldConstants.EXPOSEDFIELD, "MFNode", "face");
        ...
        idx = new Integer(FIELD_VERTEX);
        fieldMap.put("vertex", idx);
        fieldMap.put("set_vertex", idx);
        fieldMap.put("attrib_vertex", idx);

        idx = new Integer(FIELD_FACE);
        fieldMap.put("face", idx);
        fieldMap.put("set_face", idx);
        fieldMap.put("attrib_face", idx);
     ...
     */
    protected BaseShellBREP() {
        super("ShellBREP");

        hasChanged = new boolean[NUM_FIELDS];

        vfVertex = new ArrayList<VRMLNodeType>();

        vfFace = new ArrayList<VRMLNodeType>();
     ...
     */
    public BaseShellBREP(VRMLNodeType node) {
        this();
    ...
    public int[] getSecondaryType() {
        return new int[]{TypeConstants.BREPShellBREPType};
    }
    ...
    public VRMLFieldData getFieldValue(int index) throws InvalidFieldException {
        ...
        case FIELD_FACE:
            kids= new VRMLNodeType[vfFace.size()];
            vfFace.toArray(kids);
            fieldData.clear();
            fieldData.nodeArrayValue = kids;
            fieldData.dataType = VRMLFieldData.NODE_ARRAY_DATA;
    ...
    public void setValue(int index, float value) throws InvalidFieldException,
        ...
        case FIELD_FACE:
            if (!inSetup)
                throw new InvalidFieldAccessException(
                        "Cannot set initializeOnly field face after startup");
            return;
    ...
    public void setValue(int index, VRMLNodeType child)
        ...
        case FIELD_VERTEX:
            if (child instanceof VRMLCoordinateNodeType)
            {
                ...
                throw new InvalidNodeTypeException(BAD_NODE_MSG);


            break;

        case FIELD_FACE:
            if (child instanceof VRMLBREPFaceNode)
            {
                ((BaseFace)child).set_renderer(renderer);
                renderer.addRelation(this, child);
                vfFace.add(child);
            }
            else
                throw new InvalidNodeTypeException(BAD_NODE_MSG+" "+child);
            break;
        }
    }
