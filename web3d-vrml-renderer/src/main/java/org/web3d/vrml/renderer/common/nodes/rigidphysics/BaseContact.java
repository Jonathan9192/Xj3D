...
//        odeContact.setMode(surfaceModeFlags, contactIndex);
        ...
        int body_id = (int) odeContact.getBodyID1(contactIndex);
        vfBody1 = (VRMLRigidBodyNodeType)bodyIdMap.get(body_id);
        pBody1 = null;

        body_id = (int) odeContact.getBodyID2(contactIndex);
        vfBody2 = (VRMLRigidBodyNodeType)bodyIdMap.get(body_id);
        pBody2 = null;

        int geom_id = (int) odeContact.getGeomID1(contactIndex);
        vfGeometry1 = (VRMLNBodyCollidableNodeType)geomIdMap.get(geom_id);
        pGeometry1 = null;

        geom_id = (int) odeContact.getGeomID2(contactIndex);
...
//                int addr = o_body.getNativeAddr();
                ...
                int addr = (int) o_body.getGeom().getNativeAddr();
    ...
    private void setGeometry1(VRMLNodeType geom)
        ...
        if(!inSetup) {
            ...
            if(vfGeometry1 != null) {
                ...
                int addr = (int) o_geom.getNativeAddr();
