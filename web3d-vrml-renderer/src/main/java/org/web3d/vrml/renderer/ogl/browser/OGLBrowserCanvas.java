...
public class OGLBrowserCanvas
        implements BrowserCoreListener, Runnable {
    ...
    /**
     * Property defining a network protocol handler implementation for DIS
     * protocol instances. This should name a class that implements the
     * {@link org.xj3d.core.eventmodel.NetworkProtocolHandler} interface.
     */
    public static final String DIS_PROTOCOL_HANDLER_PROP =
        "org.web3d.xj3d.dis.protocol.handler.class";

    /** Default class for the DIS protocol handler */
    private static final String DEFAULT_DIS_PROTOCOL_HANDLER =
    ...
    private static final String NO_PICKING_MGR_MSG =
            "The class that you specificed for the picking manager is not a " +
            "valid instance of OGLPickingManager";

    private static final String NO_NETWORK_MGR_MSG =
            "The class that you specificed for the network manager is not a " +
    ...
    private static final String FAILED_TO_LOAD_ERR_MSG =
            "Error during the load process for the following world URL: ";
    ...
    /** The Network Protocol Handler */
    private NetworkProtocolHandler networkProtocolHandler;
    ...
    /**
     * Construct an empty canvas that contains a single view
    ...
    public void setSurface(GraphicsOutputDevice surface,
        ...
        GraphicsResizeListener[] listeners =
            universe.getGraphicsResizeListeners();
    ...
    public void initialize() {
        ...
        String[] props = (String[]) AccessController.doPrivileged(
            new PrivilegedAction<Object>() {
                public Object run() {
                    ...
                    ret_val[13] = System.getProperty(DIS_PROTOCOL_HANDLER_PROP,
        ...
        networkProtocolHandler = (NetworkProtocolHandler) loadClass(props[13],
                false,
                DIS_PROTOCOL_HANDLER_NOT_FOUND_ERR_MSG);
        ...
        // rem: origin manager
        if (sensorManager != null) {
                // create the manager for this browser instance
                sensorManager.setOriginManager(OriginManagerFactory.getInstance(stateManager));
        }
        ////////////////////////////////////////////////////////////////////////
     ...
     * process otherwise.
     *
     * @param url The URL to attempt to load in preference order
     */
    public void loadWorld(String url) {
        ...
        try {
            ...
            String msg = FAILED_TO_LOAD_ERR_MSG + url;
        ...
        int ref_loc = url.indexOf("#");
        if(ref_loc > -1)
            vp_url = url.substring(ref_loc + 1);

        universe.setScene(parsed_scene, vp_url);
    }
     ...
     * currently in use.
     * @param isShutdownThread
     */
    private void shutdownApp(boolean isShutdownThread) {

	OriginManagerFactory.removeInstance(stateManager);
