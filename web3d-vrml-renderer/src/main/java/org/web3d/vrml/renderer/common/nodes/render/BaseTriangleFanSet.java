 ...
 * <p>
 *
 * A fanSet adds the fanCount field over the normal TriangleSet.
 *
 * @author Justin Couch, Alan Hudson
 * @version $Revision: 1.12 $
 */
public abstract class BaseTriangleFanSet extends BaseTriangleGeometryNode {

    /** Index of the fanCount field */
    protected static final int FIELD_FAN_COUNT = LAST_GEOMETRY_INDEX + 1;

    /** Last index in this node */
    protected static final int LAST_TRIANGLE_INDEX = FIELD_FAN_COUNT;
    ...
    private static final String BAD_COUNT_MSG =
        "Fan count values less that 3 are not permitted. Offending value " +
        "found at index: ";
    ...
    /** field MFInt32 fanCount */
    protected int[] vfFanCount;

    /** Number of valid values in the fan count field */
    protected int numFanCount;
    ...
    static {
        ...
        idx = new Integer(FIELD_FAN_COUNT);
        fieldMap.put("fanCount", idx);
        fieldMap.put("set_fanCount", idx);
        fieldMap.put("fanCount_changed", idx);
     ...
     */
    protected BaseTriangleFanSet() {
        super("TriangleFanSet");

        vfFanCount = FieldConstants.EMPTY_MFINT32;
     ...
     */
    protected BaseTriangleFanSet(VRMLNodeType node) {
        this();
        ...
        try {
            int index = node.getFieldIndex("fanCount");
            VRMLFieldData field = node.getFieldValue(index);

            if(field.numElements != 0) {
                vfFanCount = new int[field.numElements];
    ...
    public void sendRoute(double time,
        ...
        switch(srcIndex) {
            case FIELD_FAN_COUNT:
                destNode.setValue(destIndex, vfFanCount, numFanCount);
                break;
    ...
    public VRMLFieldData getFieldValue(int index) throws InvalidFieldException {
        ...
        switch(index) {
            case FIELD_FAN_COUNT:
                fieldData.clear();
                fieldData.intArrayValue = vfFanCount;
                fieldData.numElements = numFanCount;
    ...
    /**
     * Set the value of the fanCount field.
     *
    ...
    protected void setFanCount(int[] counts, int numValid)
        ...
        if(vfFanCount.length < numValid)
            vfFanCount = new int[numValid];

        System.arraycopy(counts, 0, vfFanCount, 0, numValid);
        numFanCount = numValid;

        if(!inSetup) {
            hasChanged[FIELD_FAN_COUNT] = true;
            fireFieldChanged(FIELD_FAN_COUNT);
