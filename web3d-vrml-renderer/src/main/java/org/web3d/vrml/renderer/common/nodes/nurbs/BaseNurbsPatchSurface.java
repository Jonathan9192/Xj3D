 ...
 */
public abstract class BaseNurbsPatchSurface extends AbstractNode
    implements VRMLParametricGeometryNodeType, VRMLBREPSurfaceNode, VRMLBREPNodeType {
    ...
    protected static final int FIELD_UCLOSED = LAST_NODE_INDEX + 13;

    /** Field index for vClosed */
    protected static final int FIELD_VCLOSED = LAST_NODE_INDEX + 14;
    ...
    static {
        ...
        fieldMap.put("uClosed", new Integer(FIELD_UCLOSED));
        fieldMap.put("vClosed", new Integer(FIELD_VCLOSED));
    }

    /**
     * Create a new default instance of the node.
     */
    protected BaseNurbsPatchSurface() {
     ...
     */
//     public BaseNurbsPatchSurface(VRMLNodeType node) {
//         this();
//
//         checkNodeType(node);
//
...
//     }
    ...
    public void setupFinished() {
        ...
        if(vfCoord != null && vfCoord.getNumPoints() > 0){
           float[] control_points = new float[vfCoord.getNumPoints()];
           vfCoord.getPoint(control_points);
           generator.setPatchControlPoints(control_points,
                                           vfUDimension,
                                           vfVDimension);
        }
        else
        ...
        /*
        Note added 24 Feb 2012 by VJM
        The generator instance cannot be used for splines whose degree is 1 (linear)
        I believe this is because these generator classes offer smoothing functions which
        don't make sense for linear splines.

        I have added another test to avoid setting PatchKnots if one of the spline orders ==2
        */
        if(vfUKnot != null && vfVKnot != null &&
          (vfUKnot.length >= (vfUDimension + vfUOrder)) &&
          (vfVKnot.length >= (vfVDimension + vfVOrder)) &&
          (vfVOrder != 2) && (vfUOrder != 2))
...
//                    throw new InvalidFieldValueException("vOrder < 2: "+value);
    ...
    private void updateFacetCount() {
        ...
        if (w_facets < 3) w_facets=3;
        if (d_facets < 3) d_facets=3;

        //System.out.println("BaseNurbsPatchSurface.updateFacetCount: w_facet: "+ w_facets + "  d_facets:" + d_facets);
    ...
    //meant to be overloaded, related to compilation order issue and CADRenderer OGL node impl dependencies
