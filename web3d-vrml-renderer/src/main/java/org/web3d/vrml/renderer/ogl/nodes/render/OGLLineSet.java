...
public class OGLLineSet extends BaseLineSet
    ...
    public void updateNodeDataChanges(Object src) {
        if(vfColor == null) {
            ...
            if (num_points * 3 == num_colors) {
                implGeom.setColors(alpha, tmpColors);
            } else {
                // TODO: What to do here.  Crashes currently on 0 length
                // Don't want to issue a warning each time
                if (num_colors != 0) {
                    errorReporter.warningReport("Invalid number of colors.  Expected: " + (num_points * 3) +
                                                " got: " + num_colors, null);
