...
public abstract class BasePickingNode extends BaseSensorNode
    ...
    protected static final Map<String, Integer> sortTypeMap;

    /** Map containing the string to constant mapping for intersection type */
    protected static final Map<String, Integer> intersectTypeMap;

    /** Error message for when an invalid geometry type is received */
    ...
    private List<VRMLNodeType> targetList;
     ...
     */
    protected HashSet<String> validGeometryNodeNames;
     ...
     */
    static {
        sortTypeMap = new HashMap<String, Integer>();
        ...
        intersectTypeMap = new HashMap<String, Integer>();
    ...
    protected void copy(VRMLPickingSensorNodeType node) {
        ...
        try {
            ...
            Integer i = sortTypeMap.get(vfSortOrder);
            sortType = i.intValue();
            ...
            i = sortTypeMap.get(vfIntersectionType);
    ...
    public void setupFinished() {
        ...
        for(int i = 0; i < numPickTarget; i++) {
            VRMLNodeType p = targetList.get(i);
            p.setupFinished();
            vfPickTarget[i] = p;
        }
            ...
            case FIELD_INTERSECTION_TYPE:
                if(!inSetup)
                    throw new InvalidFieldAccessException("intersectionType can only " +
                                                          "be set at runtime");
                Integer i = intersectTypeMap.get(value);
     ...
     */
    private void init() {
        targetList = new ArrayList<VRMLNodeType>();
        ...
        validGeometryNodeNames = new HashSet<String>();
    }
     ...
     */
    private void processPickedGeometry(int numPicks, VRMLNode[] geom) {
