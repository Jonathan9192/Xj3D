...
//		errorReporter.messageReport("New CAD Kernel Instance");
		...
		faceImpl =new HashMap<VRMLNodeType, Geometry>();
	}

	/* (non-Javadoc)
	 * @see org.web3d.vrml.renderer.ogl.nodes.cadgeometry.BaseCADKernelRenderer#getBREPVertices()
	 */
	public ArrayList<BaseCoordinate> getBREPVertices() {
		ArrayList<BaseCoordinate> coord = new ArrayList<BaseCoordinate>();
		ArrayList<VRMLNode> children = this.getLinkedObjectsFrom(brepRoot);
	 ...
	 * @see org.web3d.vrml.renderer.ogl.nodes.cadgeometry.BaseCADKernelRenderer#getBREPVertices()
	 */
	public ArrayList<VRMLBREPCurve3DNode> getBREPCurves3D() {
		ArrayList<VRMLBREPCurve3DNode> curves3d = new ArrayList<VRMLBREPCurve3DNode>();
		ArrayList<VRMLNode> children = this.getLinkedObjectsFrom(brepRoot);
...
//				}
	...
	private ArrayList<VRMLBREPCurve2DNode> getPcurves(VRMLBREPFaceNode f) {
		//parse BREP tree to get pcurves from Face
		ArrayList<VRMLBREPCurve2DNode> pcurves = new ArrayList<VRMLBREPCurve2DNode>();
...
//										+ n.hashCode());
			...
			if (ga != null) {
				...
				ga.getCoordinates(0, coordinates);
					...
					if (ga instanceof IndexedGeometryArray) {
						...
						{
							int [] indexesTriangles= this.convertQuadIndexesToTriangleIndexes(indexes);
					        ...
					        for(int i=0;i<faceNormals.length;i++)
					        	for(int j=0;j<faceNormals[0].length;j++)
					        		faceNormals_1D[k++]=faceNormals[i][j];
					        ((IndexedVertexGeometry)nurbsMesh).setNormals(faceNormals_1D);
						}
						else
	...
	public void renderWireBREP() {
		...
		ArrayList<VRMLBREPCurve3DNode> curves = this.getBREPCurves3D();
		...
		((LineStripArray) impl).setVertices(VertexGeometry.COORDINATE_3,
				coords_array);
	...
	public void renderPointBREP() {
		ArrayList<BaseCoordinate> coord = this.getBREPVertices();
		...
		((PointArrayBREP) impl).setVertices(VertexGeometry.COORDINATE_3,
				coordinates_array);
	}
	...
	public VertexGeometry getEmptyImpl()
	 {
		 if (typeMesh == MeshedNurbsSurface.TesselationType.QUADS_TRIM) {
			return new org.j3d.aviatrix3d.IndexedTriangleArray();
		}
		 ...
		 if (typeMesh == MeshedNurbsSurface.TesselationType.NO_TRIM) {
			return new org.j3d.aviatrix3d.QuadArray();
		}
