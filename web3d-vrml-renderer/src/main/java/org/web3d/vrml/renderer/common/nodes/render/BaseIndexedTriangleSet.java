...
public abstract class BaseIndexedTriangleSet extends BaseIndexedTriangleGeometryNode {
    ...
    private static final String MAX_INDEX_COUNT_MSG =
        "Max coordIndex value > number of coords in IndexedTriangleSet";
     ...
     */
    protected BaseIndexedTriangleSet(VRMLNodeType node) {
        this();
    ...
    protected void updateIndexMaps() {
        ...
        for(int i = 0; i < numIndex; i++) {
            if(vfIndex[i] > maxIndexValue)
                maxIndexValue = vfIndex[i];
        }
