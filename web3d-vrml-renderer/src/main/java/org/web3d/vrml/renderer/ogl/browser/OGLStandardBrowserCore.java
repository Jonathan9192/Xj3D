...
public class OGLStandardBrowserCore
    ...
    public OGLStandardBrowserCore(EventModelEvaluator eme,
        ...
        eventModel = eme;
        eventModel.setInitListener(OGLStandardBrowserCore.this);
        ...
        coreListeners = new ArrayList<BrowserCoreListener>();

        sensorStatusListeners = new ArrayList<SensorStatusListener>(1);
        viewpointStatusListeners = new ArrayList<ViewpointStatusListener>(1);
    ...
    public void changeWorld() {
        if(currentScene != null) {
            // TODO: Can't disable as changeWorld in VRMLUniverse expects
            // events, just throttle
            ...
            ret_val = (String)AccessController.doPrivileged(
                new PrivilegedAction<Object>() {
                    public Object run() {
                        ...
                        // Do not use URL.toExternalForm as its file:/ is
                        // not right for us
                        ret_val = ret_val.replace('\\', '/');

                        // Linux and Max already start with a leading / so
                        // don't add too many
...
//        sceneManager.setEnabled(true);
    ...
    /**
     * Sync UI updates with the Application thread.  This method allows the core
    ...
    public void syncUIUpdates() {
        ...
        if(screenCaptureOnceChanged ||
           screenCaptureStartChanged ||
           screenCaptureEndChanged) {

            OGLLayerManager top_layer =
                allManagers[renderOrder[renderOrder.length - 1]];
            Viewport viewport =
                allLayers[renderOrder[renderOrder.length - 1]].getViewport();

            int width = viewport.getWidth();
            int height = viewport.getHeight();

            if (screenCaptureOnceChanged) {
                top_layer.captureScreenOnce(screenCaptureListener, width, height);
                screenCaptureOnceChanged = false;
                screenCaptureListener = null;
            }

            if (screenCaptureStartChanged) {
                top_layer.captureScreenStart(screenCaptureListener, width, height);
                screenCaptureStartChanged = false;
                screenCaptureListener = null;
            }

            if (screenCaptureEndChanged) {
                top_layer.captureScreenEnd();
        ...
        for(int i = 0; i < numLayers; i++) {
            ...
            for(int j = 0; j < navigationStateListeners.size(); j++) {
                NavigationStateListener sl =
                    navigationStateListeners.get(j);
            ...
            for(int j = 0; j < viewpointStatusListeners.size(); j++) {
                ViewpointStatusListener sl =
                    viewpointStatusListeners.get(j);
     ...
     * managing layers. The listener can be registered with the drawing surface
