...
public abstract class BaseLOD extends BaseGroupingNode
    ...
    static {
        ...
        fieldMap.put("center",FIELD_CENTER);
        fieldMap.put("range",FIELD_RANGE);

        fieldMap.put("level_changed",FIELD_LEVEL_CHANGED);
        fieldMap.put("forceTransitions",FIELD_FORCE_TRANSITIONS);
    }
    ...
    public int getFieldIndex(String fieldName) {
        ...
        // level was changed to children in V3.0 so don't allow
        // its usage in X3D
        if((ret_val == FIELD_CHILDREN) &&
           ((vrmlMajorVersion == 3) &&
           (fieldName.equals("level")))) {

            // Removed this check as it keep filter chain content from converting
            // VRML to X3D.

            //ret_val = -1;
        } else if((ret_val == FIELD_CHILDREN) &&
           ((vrmlMajorVersion == 2) &&
