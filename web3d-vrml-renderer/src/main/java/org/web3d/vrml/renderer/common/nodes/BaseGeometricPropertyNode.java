...
public abstract class BaseGeometricPropertyNode extends AbstractNode
    ...
    private List<VRMLNodeComponentListener> listeners;
    ...
    public BaseGeometricPropertyNode(String name) {
        ...
        listeners = new ArrayList<VRMLNodeComponentListener>(1);
    }
    ...
    /**
     * Fire a coordinate change event to the listeners. When
    ...
    protected void fireComponentChanged(int index) {
        ...
        for(VRMLNodeComponentListener l : listeners) {
            try {
                l.fieldChanged(this, index);
            } catch(Exception e) {
                System.err.println("Error sending component changed message: "
                                   + e.getMessage());
                e.printStackTrace();
            }
        }
