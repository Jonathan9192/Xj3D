...
public class BaseMovieTexture extends BaseTimeControlledNode
    ...
    protected List<VRMLTextureListener> textureListeners;
    ...
    public BaseMovieTexture() {
        ...
        urlListeners = new ArrayList<VRMLUrlListener>();
        contentListeners = new ArrayList<VRMLContentStateListener>();
        textureListeners = new ArrayList<VRMLTextureListener>();
...
*/
     ...
     *
     * @param duration - number of seconds the stream runs for, or -1 if unknown.
     */
    ...
    protected void fireTextureImageChanged(int idx, VRMLTextureNodeType node, NIOBufferImage image, String url) {
        ...
        for(int i = 0; i < size; i++) {
            try {
                l = textureListeners.get(i);
                l.textureImageChanged(idx, node, image, url);
            } catch(Exception e) {
                System.err.println("Error sending textureImpl changed message: "
                                   + e.getMessage());
                e.printStackTrace();
            }
        }
