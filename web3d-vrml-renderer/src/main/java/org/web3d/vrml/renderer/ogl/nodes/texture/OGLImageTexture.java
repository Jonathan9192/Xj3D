...
public class OGLImageTexture extends BaseImageTexture
     ...
     * by the internal representation to the form it needs. This assumes at
     * least some amount of intelligence on the part of the caller, but we also
     * know that we should not pass something dumb to it when we can check what
     * sort of content types it likes to handle. We assume the loader thread is
     * operating in the same context as the one that created the node in the
     * first place and thus knows the general types of items to pass through.
     *
