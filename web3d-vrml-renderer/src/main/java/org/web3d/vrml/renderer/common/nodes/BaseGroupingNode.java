...
public abstract class BaseGroupingNode extends AbstractNode
    ...
    /**
     * Error message when the child node being added is not an instance of
     * X3DChildNode.
     */
    private static final String INVALID_CHILD_NODE_MSG =
        "Attempt to add a child node to a grouping node that is not of " +
        "type X3DChildNode: ";
    ...
    public void setShared(boolean used) {
        ...
        for(int i = 0; i < num_kids; i++) {
            ...
            if(kid instanceof VRMLBoundedNodeType)
                ((VRMLBoundedNodeType)kid).setShared(used);
        }
    }
    ...
    protected void addChildNode(VRMLNodeType node)
        ...
        if(node instanceof VRMLBoundedNodeType) {
            ...
            if(impl != null && !(impl instanceof VRMLChildNodeType)) {
                throw new InvalidFieldValueException(INVALID_CHILD_NODE_MSG +
                                                     impl.getVRMLNodeName());
            }
