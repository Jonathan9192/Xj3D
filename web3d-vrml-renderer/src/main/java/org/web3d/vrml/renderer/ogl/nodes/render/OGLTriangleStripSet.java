 ...
 * <p>
 *
 * The point set directly maps to Aviatrix3D's TriangleStripArray class. When the
 ...
 */
public class OGLTriangleStripSet extends BaseTriangleStripSet
     ...
     */
    public OGLTriangleStripSet(VRMLNodeType node) {
        super(node);
    ...
    public void updateNodeBoundsChanges(Object src) {
        ...
        if(stripsChanged) {
            if (indexed) {
                TriangleStripArray tsa = (TriangleStripArray) implGeom;
                tsa.setStripCount(vfStripCount, numStripCount);
            }
    ...
    protected void buildImpl() {
        ...
        if (vfNormal != null) {
            ...
            if(((changeFlags & TEXCOORDS_CHANGED) != 0)) {
                ...
                if (texCoords == null || texCoords.length != numTexSets)
                    texCoords = new float[numTexSets][];
                ...
                for(int i = 0; i < numTexSets; i++) {
                    ...
                    if(texSetMap[i] == i) {
                        texCoords[i] = new float[max_set_size];
                        vfTexCoord.getPoint(i, texCoords[i]);
                    }
                }
            }
        } else {
            ...
            if (stripsChanged) {
                ...
                for(int i=0; i < numStripCount; i++) {
                    size += (vfStripCount[i] - 2) * 4;
                }
                ...
                for(int i=0; i < numStripCount; i++) {
                    ...
                    for(int j=0; j < vfStripCount[i] - 2; j++) {
                        if (flip == 0) {
    ...
    protected void setStripCount(int[] counts, int numValid)
        ...
        super.setStripCount(counts, numValid);
