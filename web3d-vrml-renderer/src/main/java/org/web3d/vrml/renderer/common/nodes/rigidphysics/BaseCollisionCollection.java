...
public abstract class BaseCollisionCollection extends AbstractNode
    ...
    protected static final Map<String, Integer> parameterIdMap;
     ...
     */
    protected static final Map<String, Integer> internalParamIdMap;

    // The VRML field values

    /** The value of the collidables field */
    private List<VRMLNodeType> vfCollidables;
    ...
    private List<VRMLNBodySpaceNodeType> spaceChildren;

    /** List of nodes that we've determined are child geometry */
    private List<VRMLNBodyCollidableNodeType> geomChildren;
    ...
    static {
        ...
        parameterIdMap = new HashMap<String, Integer>();
        ...
        internalParamIdMap = new HashMap<String, Integer>();
    ...
    public BaseCollisionCollection() {
        ...
        vfCollidables = new ArrayList<VRMLNodeType>();
        vfEnabled = true;
        ...
        spaceChildren = new ArrayList<VRMLNBodySpaceNodeType>();
        geomChildren = new ArrayList<VRMLNBodyCollidableNodeType>();
...
// So, we have to manually do this calling the check on all contained spaces.
    ...
    public void setupFinished() {
        ...
        for(int i = 0; i < size; i++) {
            VRMLNBodySpaceNodeType n = spaceChildren.get(i);
            n.setParentODESpace(rootSpace);
        }

        size = vfCollidables.size();
        for(int i = 0; i < size; i++) {
            VRMLNodeType n = vfCollidables.get(i);
            if(n != null)
                n.setupFinished();
        }
        ...
        for(int i = 0; i < size; i++) {
            VRMLNBodyCollidableNodeType n = geomChildren.get(i);
    ...
    private void setAppliedParameters(String[] value, int numValid) {
        ...
        for(int i = 0; i < numValid; i++) {
            Integer val = parameterIdMap.get(value[i]);

            if(val != null) {
                surfaceModeFlags |= val.intValue();

                val = internalParamIdMap.get(value[i]);
    ...
    protected void addCollidable(VRMLNodeType node)
        ...
        if(node instanceof VRMLNBodyCollidableNodeType) {
            ...
            geomChildren.add((VRMLNBodyCollidableNodeType) node);
        } else if(node instanceof VRMLNBodySpaceNodeType) {
            vfCollidables.add(node);
            spaceChildren.add((VRMLNBodySpaceNodeType) node);
            ...
            if(impl instanceof VRMLNBodyCollidableNodeType) {
                ...
                geomChildren.add((VRMLNBodyCollidableNodeType) impl);
            } else if(impl instanceof VRMLNBodySpaceNodeType) {
                vfCollidables.add(impl);
                spaceChildren.add((VRMLNBodySpaceNodeType) impl);
            } else
