...
public abstract class BaseGeoLOD extends BaseGroupingNode
    ...
    private static final HashSet<String> validX3DTypes;
    ...
    protected List<VRMLNodeType> vfRootNode;
    ...
    static {
        ...
        validX3DTypes = new HashSet<String>();
    ...
    protected BaseGeoLOD() {
        ...
        vfRootNode = new ArrayList<VRMLNodeType>();
        localCenter = new double[3];
        urlRelativeCheck = false;
        loadState = new int[NUM_FIELDS];
        loadedUri = new String[NUM_FIELDS];
        urlListeners = new ArrayList<VRMLUrlListener>(1);
        contentListeners = new ArrayList<VRMLContentStateListener>(1);
...
*/
     ...
     *
     * @param index THe field index for the preferred types.
     * @return A list of preferred class types
    ...
    public void setupFinished() {
        ...
        try {
            ...
            center[0] = localCenter[0];
            center[1] = localCenter[1];
            center[2] = localCenter[2];
            ...
            if (local_origin != null) {
                localCenter[0] -= local_origin[0];
                localCenter[1] -= local_origin[1];
                localCenter[2] -= local_origin[2];
            }
    ...
    public void setValue(int index, float value)
        ...
        switch(index) {
            case FIELD_RANGE:
                if(!inSetup)
                    throwInitOnlyWriteException("range");
    ...
    public void setValue(int index, double[] value, int numValid)
        ...
        switch(index) {
            case FIELD_CENTER:
                if(!inSetup)
                   throwInitOnlyWriteException("center");
    ...
    public void setValue(int index, String[] value, int numValid)
        ...
        switch(index) {
            ...
            case FIELD_ROOT_URL:
                if(!inSetup)
                    throwInitOnlyWriteException("rootUrl");

                if(vfRootUrl.length != numValid)
                    vfRootUrl= new String[numValid];
                System.arraycopy(value, 0, vfRootUrl, 0, numValid);
                break;

            case FIELD_CHILD1_URL:
                if(!inSetup)
                    throwInitOnlyWriteException("child1Url");

                if(vfChild1Url.length != numValid)
                    vfChild1Url = new String[numValid];
                System.arraycopy(value, 0, vfChild1Url, 0, numValid);
                break;

            case FIELD_CHILD2_URL:
                if(!inSetup)
                    throwInitOnlyWriteException("child2Url");

                if(vfChild2Url.length != numValid)
                    vfChild2Url = new String[numValid];
                System.arraycopy(value, 0, vfChild2Url, 0, numValid);
                break;

            case FIELD_CHILD3_URL:
                if(!inSetup)
                    throwInitOnlyWriteException("child3Url");

                if(vfChild3Url.length != numValid)
                    vfChild3Url = new String[numValid];
                System.arraycopy(value, 0, vfChild3Url, 0, numValid);
                break;

            case FIELD_CHILD4_URL:
                if(!inSetup)
                    throwInitOnlyWriteException("child4Url");

                if(vfChild4Url.length != numValid)
                    vfChild4Url = new String[numValid];
                System.arraycopy(value, 0, vfChild4Url, 0, numValid);
                break;
    ...
    public void setValue(int index, VRMLNodeType child)
        ...
        switch(index) {
            ...
            case FIELD_ROOT_NODE:
                if(!inSetup)
                    throwInitOnlyWriteException("rootNode");
    ...
    public void originChanged() {
        ...
        if (origin == null) {
            ...
            localCenter[0] = center[0] - local_origin[0];
            localCenter[1] = center[1] - local_origin[1];
            localCenter[2] = center[2] - local_origin[2];
        }
    }
    ...
    protected void loadScene(boolean children)
        ...
        if (childrenShown) {
            ...
            if((vfChild2Url.length != 0)&&(loadedScenes[1] == null)) {
                fireUrlChanged(FIELD_CHILD2_URL);
            }
            if((vfChild3Url.length != 0)&&(loadedScenes[2] == null)) {
                fireUrlChanged(FIELD_CHILD3_URL);
            }
            if((vfChild4Url.length != 0)&&(loadedScenes[3] == null)) {
                fireUrlChanged(FIELD_CHILD4_URL);
            }

        } else if (vfRootNode.isEmpty()) {
            // using the root url
