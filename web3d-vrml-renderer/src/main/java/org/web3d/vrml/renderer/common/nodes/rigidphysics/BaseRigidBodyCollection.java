...
public class BaseRigidBodyCollection extends AbstractNode
    ...
    protected ArrayList<BaseContact> vfSetContacts;
    ...
    private List<VRMLRigidBodyNodeType> bodyNodes;
    ...
    public BaseRigidBodyCollection() {
        ...
        vfSetContacts = new ArrayList<BaseContact>();

        odeJointGroup = new JointGroup();
        bodyNodes = new ArrayList<VRMLRigidBodyNodeType>();
    ...
    public void processInputContacts() {
        ...
        if((vfSetContacts.isEmpty()) || (vfCollider == null))
            return;
        ...
        for(int i = 0; i < num_contacts; i++) {
            BaseContact contact = vfSetContacts.get(i);
    ...
    public void evaluateModel() {
        ...
        for(int i = 0; i < size; i++) {
            VRMLRigidBodyNodeType r_body = bodyNodes.get(i);
    ...
    private void setContacts(VRMLNodeType[] children, int numValid) {
        ...
        for(int i = 0; i < numValid; i++) {
            if(children[i] instanceof BaseContact)
                vfSetContacts.add((BaseContact)children[i]);
                ...
                vfSetContacts.add((BaseContact)impl);
    ...
    private void addBody(VRMLNodeType child)
        ...
        if(child instanceof VRMLProtoInstance) {
            ...
            bodyNodes.add((VRMLRigidBodyNodeType)impl);

        } else if((child != null) && !(child instanceof VRMLRigidBodyNodeType))
            throw new InvalidFieldValueException(INVALID_BODY_NODE_MSG);
        else
            bodyNodes.add((VRMLRigidBodyNodeType)child);
