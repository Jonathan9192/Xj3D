...
class ViewpointResizeManager
    ...
    private List<ViewpointData> perspectiveViewpoints;

    /** The list of fixed size viewports to manage */
    private List<ViewpointData> orthoViewpoints;

    /** The mapping of a node to its ViewportData type. */
    private Map<VRMLViewpointNodeType, ViewpointData> nodeDataMap;
    ...
    ViewpointResizeManager() {
        perspectiveViewpoints = new ArrayList<ViewpointData>();
        orthoViewpoints = new ArrayList<ViewpointData>();

        nodeDataMap = new HashMap<VRMLViewpointNodeType, ViewpointData>();
    ...
    void sendResizeUpdates() {
        ...
        for(int i = 0; i < perspectiveViewpoints.size(); i++) {
            processPerspectiveViewpointChange(perspectiveViewpoints.get(i));
        }

        for(int i = 0; i < orthoViewpoints.size(); i++) {
            processOrthoViewpointChange(orthoViewpoints.get(i));
        }
    ...
    void addViewpoint(VRMLViewpointNodeType node, ViewEnvironment view) {
        if((view == null) || nodeDataMap.containsKey(node))
            return;
    ...
    void removeViewpoint(VRMLViewpointNodeType node) {
        ...
        ViewpointData data = nodeDataMap.get(node);
