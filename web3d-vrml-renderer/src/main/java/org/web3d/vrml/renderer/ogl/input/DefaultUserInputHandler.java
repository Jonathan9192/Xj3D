...
public class DefaultUserInputHandler
    ...
    private static final Map<String, Integer> navigationTypes;
    ...
    private HashSet<Object> activeSensors;

    /** A temporary working variable for the active sensors */
    private HashSet<Object> workingSensors;
     ...
     * next while doing terrain following.
     */
    ...
    *  with the shift modifier active */
    ...
    /** Flags indicating the pressed state of the mouse buttons */
     ...
     */
    static {
        navigationTypes = new HashMap<String, Integer>();
        ...
        navigationTypes.put(Xj3DConstants.FLY_LEVEL_NAV_MODE,
            new Integer(Xj3DNavigationState.FLY_LEVEL_STATE));
    ...
    public DefaultUserInputHandler() {
        ...
        activeSensors = new HashSet<Object>();
        workingSensors = new HashSet<Object>();
        ...
        navigationStateListeners = new ArrayList<NavigationStateListener>(1);
        sensorStatusListeners = new ArrayList<SensorStatusListener>(1);
    ...
    public void trackerPressed(int tracker, TrackerState evt) {
        ...
        System.arraycopy(evt.buttonState, 0, buttonPressed, 0, buttonPressed.length);
            ...
            if(path == null) {
                ...
                if(!object_found) {
                    ...
                    if( hasDragSensor ) {
                        ...
                        sensorHitNormal[0] = wkPoint.x;
                        sensorHitNormal[1] = wkPoint.y;
                        sensorHitNormal[2] = wkPoint.z;
                        ...
                        sensorHitPoint[0] = pointTrans.x;
                        sensorHitPoint[1] = pointTrans.y;
                        sensorHitPoint[2] = pointTrans.z;
    ...
    public void trackerMoved(int tracker, TrackerState evt) {
        ...
        if(path == null) {
            ...
            if(size != 0) {
                ...
                tmpSensors = activeSensors.toArray(tmpSensors);
    ...
    public void trackerDragged(int tracker, TrackerState state) {
        ...
        case TOUCH_STATE:
            ...
            if ( hasDragSensor ) {
                ...
                sensorHitPoint[0] = wkPoint.x;
                sensorHitPoint[1] = wkPoint.y;
                sensorHitPoint[2] = wkPoint.z;

                sensorHitNormal[0] = diffVec.x;
                sensorHitNormal[1] = diffVec.y;
                sensorHitNormal[2] = diffVec.z;
            ...
            if ( size != 0 ) {
                ...
                tmpSensors = workingSensors.toArray(tmpSensors);
    ...
    public void trackerWheel(int tracker, TrackerState state) {
        if (wheelEnabled) {
            String current = navTypes[currentNavTypeIndex];
            if (current.equals(Xj3DConstants.EXAMINE_NAV_MODE)) {
                    tmpVec1[0] = 0;
                    tmpVec1[1] = 0;
                    ...
                    wheelMoved = true;
            } else if (current.equals(Xj3DConstants.FLY_LEVEL_NAV_MODE)) {
                    tmpVec1[0] = 0;
                    tmpVec1[1] = 0;
                    tmpVec1[2] = state.wheelClicks;
                    ...
                    navHandler.move(tmpVec1, 1);
                    wheelMoved = true;
            } else if (current.equals(Xj3DConstants.TRACK_EXAMINE_NAV_MODE)) {
                tmpVec1[0] = 0;
                tmpVec1[1] = 0;
                tmpVec1[2] = state.wheelClicks;

                navHandler.move(tmpVec1, 0.005f);
                wheelMoved = true;
            }
        }
    }
    ...
    public boolean setNavigationMode(String mode) {
        ...
        if(!found && found_any) {
            ...
            System.arraycopy(navTypes, 0, new_types, 0, navTypes.length);
        ...
        if(found) {
            ...
            typeConst = navigationTypes.get(mode);
    ...
    public void clear() {
        ...
        navHandler.clear();
        List<SceneGraphPath> pick_list = (List<SceneGraphPath>)rayPicker.foundPaths;
        if ( pick_list != null ) {
            pick_list.clear();
        }
    ...
    public void setOriginManager(OriginManager manager) {
        navHandler.setOriginManager(manager);
    }
    ...
    private SceneGraphPath doPick(TrackerState state) {
        ...
        List<SceneGraphPath> pick_results = (List<SceneGraphPath>)rayPicker.foundPaths;
        ...
        int shortest = -1;
        pick_results = (List<SceneGraphPath>)rayPicker.foundPaths;
        ...
        if(!valid_pick)
            ...
            return pick_results.get(shortest);
    }
    ...
    private void fireNavigationStateChanged(int idx) {
        ...
        for(int i = 0; i < size; i++) {
            try {
                NavigationStateListener l = navigationStateListeners.get(i);

                l.navigationStateChanged(idx);
            } catch(Exception e) {
                System.err.println("Error sending navigation state changed " + e);
                e.printStackTrace();
            }
        }
    }
    ...
    private void fireNavigationListChanged(String[] modes, int numModes) {
        ...
        for(int i = 0; i < size; i++) {
            try {
                ...
                System.err.println("Error sending navigation list changed " + e);
                e.printStackTrace();
            }
        }
    }
    ...
    private void fireDeviceOver(Object sensor) {
        ...
        for(int i = 0; i < size; i++) {
            try {
                SensorStatusListener l = sensorStatusListeners.get(i);
    ...
    private void fireLinkActivated(VRMLLinkNodeType link) {
        ...
        String[] params = link.getParameter();
        ...
        for(int i = 0; i < size; i++) {
            try {
                ...
                l.linkActivated(url, params, desc);
            } catch(Exception e) {
                System.err.println("Error sending navigation list changed " + e);
                e.printStackTrace();
            }
    ...
    private void updateCenterOfRotation(TrackerState evt) {
        SceneGraphPath path = doPick(evt);
