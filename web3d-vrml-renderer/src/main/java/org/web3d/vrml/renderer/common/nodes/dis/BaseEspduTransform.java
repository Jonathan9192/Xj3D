...
import edu.nps.moves.math.Quaternion;
...
public class BaseEspduTransform extends BaseGroupingNode
    ...
    static {
        ...
        fieldMap = new HashMap<String, Integer>(NUM_FIELDS);
        ...
        networkModes = new HashMap<String, Integer>(3);
    ...
    public BaseEspduTransform() {
        ...
        roleListeners = new ArrayList<NetworkRoleListener>(1);
...
//System.out.println("SetRotation called");
     ...
     *
     * @param index The index of destination field to set
     * @param child The new child node to use for the node
     * @throws InvalidFieldException The field index is not known
    ...
    /** @return the network address to listen to */
    public String getAddress() {
        return vfAddress;
    }

    /** @return the network port to listen to */
    public int getPort() {
        return vfPort;
    }
    ...
    /** @param active the isActive state for a DIS node */
     ...
     * accounts for local values, not required DIS heartbeat rules.
     *
    ...
    public Pdu getState() {
        ...
        for (int i = 0; i < vfArticulationParameterCount; i++) {
            ...
            espdu.getArticulationParameters().add(artVals[i]);
        }
...
//System.out.println("roll: " + roll + " pitch: " + pitch + " yaw: " + yaw);
        Orientation orien = espdu.getEntityOrientation();
    ...
    /**
     * Tell the DIS node that a packet arrived.  Used to update timestamp
     * information.
     */
...
//            case PduTypeField.FIREFI:
            case FIRE:
                ...
                vfEventApplicationID = firepdu.getEventID().getApplication();
                hasChanged[FIELD_EVENT_APPLICATION_ID] = true;
                fireFieldChanged(FIELD_EVENT_APPLICATION_ID);

                vfEventEntityID = firepdu.getEventID().getEventNumber();
                hasChanged[FIELD_EVENT_ENTITY_ID] = true;
                fireFieldChanged(FIELD_EVENT_ENTITY_ID);

                vfEventSiteID = firepdu.getEventID().getSite();
                hasChanged[FIELD_EVENT_SITE_ID] = true;
                fireFieldChanged(FIELD_EVENT_SITE_ID);

                vfMunitionApplicationID = firepdu.getMunitionID().getApplication();
                hasChanged[FIELD_MUNITION_APPLICATION_ID] = true;
                fireFieldChanged(FIELD_MUNITION_APPLICATION_ID);

                vfMunitionEntityID = firepdu.getMunitionID().getEntity();
                hasChanged[FIELD_MUNITION_ENTITY_ID] = true;
                fireFieldChanged(FIELD_MUNITION_ENTITY_ID);

                vfMunitionSiteID = firepdu.getMunitionID().getSite();
                hasChanged[FIELD_MUNITION_SITE_ID] = true;
                fireFieldChanged(FIELD_MUNITION_SITE_ID);
                ...
                vfFiringRate = firepdu.getBurstDescriptor().getRate();
                hasChanged[FIELD_FIRING_RATE] = true;
                fireFieldChanged(FIELD_FIRING_RATE);

                vfFiringRange = firepdu.getRangeToTarget();
                hasChanged[FIELD_FIRING_RANGE] = true;
                fireFieldChanged(FIELD_FIRING_RANGE);
...
//            case PduTypeField.DETONATIONFI:
            case DETONATION:
                ...
                vfDetonationResult = dtpdu.getDetonationResult();
                hasChanged[FIELD_DETONATION_RESULT] = true;
                fireFieldChanged(FIELD_DETONATION_RESULT);

                vfDetonationRelativeLocation[0] = dtpdu.getLocationInEntityCoordinates().getX();
                vfDetonationRelativeLocation[1] = -dtpdu.getLocationInEntityCoordinates().getZ();
                vfDetonationRelativeLocation[2] = dtpdu.getLocationInEntityCoordinates().getY();
                hasChanged[FIELD_DETONATION_RELATIVE_LOCATION] = true;
                fireFieldChanged(FIELD_DETONATION_RELATIVE_LOCATION);
...
//System.out.println("recv vfRot: " + vfRotation[0] + " " + vfRotation[1] + " " + vfRotation[2] + " " + vfRotation[3]);
                ...
                vfEntityDomain = localEspdu.getEntityType().getDomain();
                hasChanged[FIELD_ENTITY_DOMAIN] = true;
                fireFieldChanged(FIELD_ENTITY_DOMAIN);

                vfEntityKind = localEspdu.getEntityType().getEntityKind();
                hasChanged[FIELD_ENTITY_KIND] = true;
                fireFieldChanged(FIELD_ENTITY_KIND);

                vfForceID = localEspdu.getForceId();
                hasChanged[FIELD_FORCE_ID] = true;
                fireFieldChanged(FIELD_FORCE_ID);
                ...
                vfEntitySpecific = localEspdu.getEntityType().getSpec();
                hasChanged[FIELD_ENTITY_SPECIFIC] = true;
                fireFieldChanged(FIELD_ENTITY_SPECIFIC);
                ...
                vfEntityCountry = localEspdu.getEntityType().getCountry();
                hasChanged[FIELD_ENTITY_COUNTRY] = true;
                fireFieldChanged(FIELD_ENTITY_COUNTRY);

                int articCount = localEspdu.getArticulationParameters().size();
                ...
                if (articCount > 0) {
                    ...
                    artVals = localEspdu.getArticulationParameters().toArray(artVals);

                    for (int i = 0; i < articCount; i++) {
                        vfArticulationParameterArray[i] = (float) artVals[i].getParameterValue();
...
//System.out.println("Final Pos: " + vfTranslation[0] + " " + vfTranslation[1] + " " + vfTranslation[2]);
    ...
    public void regenID() {
        id = new EntityID();
        id.setApplication(vfApplicationID);
        id.setSite(vfSiteID);
        id.setEntity(vfEntityID);
    }
