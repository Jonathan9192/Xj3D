...
public abstract class AbstractScene implements BasicScene {
    ...
    protected HashSet<VRMLNode> allNodes;

    /** List of all protos/externprotos held by this class */
    protected List<VRMLNodeTemplate> templateList;

    /** The list of template declarations stored here by name (key) */
    protected Map<String, VRMLNodeTemplate> templateMap;
    ...
    private Map<String, VRMLNode> importMap;
    ...
    protected AbstractScene(int major, int minor) {
        ...
        allNodes = new HashSet<VRMLNode>();
        routeList = new ArrayList<ROUTE>();
        templateList = new ArrayList<VRMLNodeTemplate>();
        templateMap = new HashMap<String, VRMLNodeTemplate>();
        primaryTypes = new IntHashMap();
        secondaryTypes = new IntHashMap();
        defMap = new HashMap<String, VRMLNode>();
        importMap = new HashMap<String, VRMLNode>();
    }
    ...
    public void addNode(VRMLNode node) {
        ...
        List<VRMLNode> list = (List<VRMLNode>) primaryTypes.get(p_type);

        if(list == null) {
            list = new ArrayList<VRMLNode>();
        ...
        for(int i = 0; i < s_types.length; i++) {
            list = (List<VRMLNode>)secondaryTypes.get(s_types[i]);
    ...
    public void removeNode(VRMLNode node) {
        ...
        List list = (List)primaryTypes.get(p_type);
        list.remove(node);
     ...
     */
    public List<VRMLNodeTemplate> getNodeTemplates() {
        return templateList;
    }
     ...
     */
    public List<VRMLNode> getByPrimaryType(int type) {
        List<VRMLNode> ret_val = (List<VRMLNode>)primaryTypes.get(type);

        if(ret_val == null) {
            ret_val = new ArrayList<VRMLNode>();
     ...
     * @return The list of node instances in this scene.
     */
    public List getBySecondaryType(int type) {
        List ret_val = (List)secondaryTypes.get(type);
     ...
     */
    public Map<String, VRMLNode> getDEFNodes() {
        return defMap;
    }
     ...
     */
    public Map<String, VRMLNode> getImports() {
        return importMap;
    }
     ...
     * already registered, it is replaced with the incoming instance (as per
     * VRML specification.
     *
     ...
     * @param node The node containing the DEF name of the local inline node
     * @param exported The exported name
     */
    public void addImportedNode(String as, VRMLNode node, String exported) {
        if(node != null) {
            if(as != null)
                importMap.put(as, node);
            else
                importMap.put(exported, node);
        }
    }

    /**
     * Remove an import name from the map.
     *
     * @param name The name of the node to be removed
     */
