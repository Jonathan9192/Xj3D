...
public abstract class CRMainSceneBuilder
     ...
     */
    protected Map<String, Map<String, VRMLNode>>exportProxyMap;
    ...
    public void startDocument(String uri,
        ...
        // VRML doesn't have a profile statement, thus the profileDecl() call will
        // never be made to initialise everything. This just makes sure that we
    ...
    public void importDecl(String inline, String exported, String imported)
        ...
        VRMLNodeType node = defMap.get(inline);
        ...
        scene.addImportedNode(imported, n, exported);
    ...
    public void exportDecl(String defName, String exported)
        ...
        // Look in the DEF map for the name.
        VRMLNodeType node = defMap.get(defName);
...
//System.out.println("endField() called: " + currentNode.getClass());
    ...
    public void routeDecl(String srcNodeName,
        ...
        VRMLNodeType src_node = defMap.get(srcNodeName);
        ...
        if(src_node == null) {
            ...
            if((majorVersion < 3) || !importProxyMap.containsKey(srcNodeName))
                throw new InvalidFieldConnectionException(
                                 ...
                                 srcNodeName + " Line: " + locator.getLineNumber() +
                " Column: " + locator.getColumnNumber());

            imported_route = true;
            src_node = importProxyMap.get(srcNodeName);
        }

        VRMLNodeType dest_node = defMap.get(destNodeName);

        if(dest_node == null) {
            if((majorVersion < 3) || !importProxyMap.containsKey(destNodeName))
                throw new InvalidFieldConnectionException(
                               "ROUTE declares an unknown destination node DEF: " +
                               destNodeName + " Line: " + locator.getLineNumber() +
                " Column: " + locator.getColumnNumber());

            imported_route = true;
            dest_node = importProxyMap.get(destNodeName);
        }
        ...
        if(dest_index == -1)
            throw new InvalidFieldConnectionException(
                           ...
                           destNodeName + "." + destFieldName + " Line: " + locator.getLineNumber() +
                           " Column: " + locator.getColumnNumber());
    ...
    protected final void resizeStack() {
        ...
        if((topOfStack + 1) == nodeStack.length) {
            ...
            System.arraycopy(childIndexStack, 0, tmp_index, 0, old_size);
    ...
    private void parseField(VRMLNodeType node,
        ...
        switch(fieldType) {
            ...
            case FieldConstants.SFVEC2D:
                d_val = fieldParser.SFVec2d(value);
                node.setValue(index, d_val, d_val.length);
                break;
