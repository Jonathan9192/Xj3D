...
public class CRProtoInstance
    ...
    protected Map<String, Integer> uniqueFieldIndexMap;
     ...
     */
    protected List<VRMLFieldDeclaration> fieldDeclList;
    ...
    protected Map<String, VRMLNode> proxyNodeMap;
    ...
    /** Flag indicating we should ignore setValues that overwrite values */
    ...
    private HashSet<Integer> activeInputFields;
    ...
    private class ISFieldAdapter implements VRMLNodeListener {
        ...
        public void fieldChanged(int index) {
            if((index != sourceIndex) || activeInputFields.contains(new Integer(index)))
                return;
    ...
    protected CRProtoInstance(String name,
        ...
        fieldIndexMap = new HashMap<String, Integer>(lastFieldIndex, 1.0f);
        uniqueFieldIndexMap = new HashMap<String, Integer>(lastFieldIndex, 1.0f);
        fieldDeclList = new ArrayList<VRMLFieldDeclaration>(lastFieldIndex);
        userData = new IntHashMap(lastFieldIndex);

        bodyNodes = new VRMLNodeType[numBodyNodes];
        fieldLocalData = new VRMLFieldDataThreaded();
        activeInputFields = new HashSet<Integer>();
        ...
        proxyNodeMap = new HashMap<String, VRMLNode>();
        delayedAdd = false;
    ...
    public void setupFinished() {
        ...
        List<VRMLNodeType> list;
        for(int i=0; i < keySet.length; i++) {
            list = (List<VRMLNodeType>) lostList.get(keySet[i]);

            for (int k=0; k < list.size(); k++) {
                VRMLNodeType node = list.get(k);
     ...
     */
     public void notifyExternProtoLoaded(int index, VRMLNodeType node)
        ...
        if (inSetup) {
            List<VRMLNodeType> list = (List<VRMLNodeType>) lostList.get(index);

            if (list == null) {
                list = new ArrayList<VRMLNodeType>();
                list.add(node);
...
// Not the right size. Should multiply by the raw data size
    ...
    public void setValue(int index, int value)
        ...
        activeInputFields.add(new Integer(index));

        for(int i = 0; i < is_list.length; i++)
            is_list[i].node.setValue(is_list[i].field, value);

        activeInputFields.remove(new Integer(index));
    }
            ...
            if (decl.getFieldType() == FieldConstants.MFNODE && inSetup && !resendingIS) {
                ...
                if (old_value == null) {
                    ...
                    if (old_value instanceof VRMLNodeType) {
                        ...
                        System.arraycopy(old_array, 0, childs, 0, cnt);
...
*/
    ...
    /**
     * Add another node to the internal definition of the node. The first item
     ...
     */
    void setImports(Map<String, VRMLNode> imports) {
     ...
     */
    void setIsMapping(int index, List<ProtoFieldInfo> destList) {
     ...
     */
    void resendIS() {
        Iterator<Integer> itr = uniqueFieldIndexMap.values().iterator();
        int idx;
