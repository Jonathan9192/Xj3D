...
public class MFFloat extends FieldScriptableObject {
    ...
    private List<ReusableFloat> valueList;
    ...
    public MFFloat() {
        ...
        valueList = new ArrayList<ReusableFloat>();
    }
     ...
     * node).
     */
    public MFFloat(float[] vals, int numValid) {
        this();

        if(numValid > 0) {
            for(int i = 0; i < numValid; i++)
                valueList.add(new ReusableFloat(vals[i]));
    ...
    public MFFloat(Object[] args) {
        ...
        for(int i=0; i < args.length; i++) {
            ...
            cnt++;
            valueList.add(new ReusableFloat(((Number)args[i]).floatValue()));
        }
    ...
    public static Scriptable jsConstructor(Context cx, Object[] args,
        ...
        MFFloat result = new MFFloat(args);
    ...
    public boolean equals(Object val) {
        if(!(val instanceof MFFloat))
            return false;

        MFFloat o = (MFFloat)val;
    ...
    /**
     * Get the array of underlying float values.
     *
     * @return An array of the values
     */
    public float[] getRawData() {
        float[] ret_val = new float[valueList.size()];

        for(int i=0; i < valueList.size();i++) {
            ret_val[i] = valueList.get(i).floatValue();
        }
