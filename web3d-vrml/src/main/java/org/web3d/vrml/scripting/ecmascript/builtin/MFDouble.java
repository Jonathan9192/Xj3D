...
public class MFDouble extends FieldScriptableObject {
    ...
    public MFDouble() {
        ...
        valueList = new ArrayList<ReusableDouble>();
    }
     ...
     * node).
     */
    public MFDouble(float[] vals, int numValid) {
        this();

        if(numValid > 0) {
            for(int i = 0; i < numValid; i++)
                valueList.add(new ReusableDouble(vals[i]));
     ...
     * node).
     */
    public MFDouble(double[] vals, int numValid) {
        this();
    ...
    public MFDouble(Object[] args) {
        ...
        for(int i=0; i < args.length; i++) {
            ...
            cnt++;
            valueList.add(new ReusableDouble(((Number)args[i]).floatValue()));
        }
