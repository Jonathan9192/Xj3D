...
public class BaseExecutionContext implements X3DExecutionContext {
    ...
    protected List<BaseNode> nodeWrapperList;

    /** Mapping of the VRMLNodeType to its X3DNode wrapper */
    protected Map<VRMLNodeType, X3DNode> nodeWrapperMap;

    /** Mapping of the ROUTE to its X3DRoute wrapper */
    protected Map<ROUTE, X3DRoute> routeWrapperMap;

    /** Map of the proto name to it's SAI representation  */
    private Map<String, X3DProtoDeclaration> protoObjectMap;

    /** Map of the externproto name to it's SAI representation  */
    private Map<String, X3DExternProtoDeclaration> externObjectMap;
     ...
     * @param fal The access listener for propagating s2 requests
    ...
    public BaseExecutionContext(VRMLExecutionSpace space,
        ...
        nodeWrapperList = new ArrayList<BaseNode>();
        nodeWrapperMap = new HashMap<VRMLNodeType, X3DNode>();
        routeWrapperMap = new HashMap<ROUTE, X3DRoute>();
        protoObjectMap = new HashMap<String, X3DProtoDeclaration>();
        externObjectMap = new HashMap<String, X3DExternProtoDeclaration>();

        SceneMetaData md = scene.getMetaData();
        specVersion = md.getVersion();
        this.url = scene.getLoadedURI();
    ...
    public BaseExecutionContext(VRMLExecutionSpace space,
        ...
        List<VRMLNodeTemplate> templates = scene.getNodeTemplates();
        int num_proto = 0;
        int num_ep = 0;
        ...
        for (int i = 0; i < size; i++) {
            VRMLNodeTemplate t = templates.get(i);
    ...
    public X3DRoute[] getRoutes() {
        List<ROUTE> src_routes = scene.getRoutes();
        int num_routes = src_routes.size();
        X3DRoute[] ret_val = new X3DRoute[num_routes];

        for(int i = 0; i < num_routes; i++) {
            ROUTE r = src_routes.get(i);
            ret_val[i] = routeWrapperMap.get(r);
            ...
            if (ret_val[i] == null) {
                ...
                if (d_xn == null) {
                    ...
                    nodeWrapperList.add(d_xn);
                }

                ret_val[i] =
                    new SAIRoute(s_xn, sf.getName(), d_xn, df.getName());
                routeWrapperMap.put(r, ret_val[i]);
            }
        }
...
//                routeWrapperMap.put(r, ret_val);
    ...
    public void removeRoute(X3DRoute route)
        ...
        int src_idx = src_node.getFieldIndex(sr.srcField);
        int dest_idx = dest_node.getFieldIndex(sr.destField);
    ...
    public String[] getProtosNames() {
        ...
        if (!protoObjectMap.isEmpty()) {
            Set<String> s = protoObjectMap.keySet();
    ...
    public X3DProtoDeclaration getProtoDeclaration(String name) {
        return protoObjectMap.get(name);
    }
    ...
    public String[] getExternProtoNames() {
        ...
        if (!externObjectMap.isEmpty()) {
            Set<String> s = externObjectMap.keySet();
            ret_val = new String[s.size()];
            s.toArray(ret_val);
        }
    ...
    public X3DNode getNamedNode(String name) {
        ...
        X3DNode ret_val = nodeWrapperMap.get(v_node);
        ...
        if (ret_val == null) {
            ...
            //                       fieldFactory,
            //                       fieldAccessListener);
            ret_val = baseNodeFactory.getBaseNode( v_node );
            ((BaseNode)ret_val).setAccessValid(true);
            nodeWrapperMap.put(v_node, ret_val);
            nodeWrapperList.add((BaseNode)ret_val);
        }
    ...
    public void updateNamedNode(String nodeName, X3DNode node) {
        VRMLNode newNode = (VRMLNode) ((BaseNode) node).getImplNode();
    ...
    public X3DNode getImportedNode(String name) {
        ...
        // Look up to see if we have a cached wrapper of this node already.
        X3DNode ret_val = nodeWrapperMap.get(importedNode);
        ...
        if (ret_val == null) {
            ...
            //                       fieldFactory,
            //                       fieldAccessListener);
            ret_val = baseNodeFactory.getBaseNode( importedNode );
            ((BaseNode)ret_val).setAccessValid(true);
    ...
    public void updateImportedNode(String exportedName,
        ...
        // Copied from the external SAI implementation.
        ...
        if (realNode instanceof VRMLInlineNodeType) {
            ...
            VRMLScene localScene = (VRMLScene) inlineNode.getContainedScene();
            VRMLNodeType foreignNode = (VRMLNodeType) localScene.getExports();

            if (foreignNode == null) {
                throw new InvalidNodeException(exportedName + " not found in target scene.");
            }

            localScene.getImports().put(importedName, foreignNode);
        } else {
    ...
    public X3DNode[] getRootNodes() {
        ...
        for(int i = 0; i < n_kids; i++) {
            ret_val[i] = nodeWrapperMap.get(kids[i]);
     ...
     * these fields that any changes made are propagated through to the
     * internals.
     */
    void updateEventOuts() {
        ...
        for(int i = 0; i < num_nodes; i++) {
            BaseNode n = nodeWrapperList.get(i);
            n.updateNodeAndChildren();
        }
    }
