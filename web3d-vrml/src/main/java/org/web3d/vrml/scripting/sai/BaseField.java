...
abstract class BaseField implements X3DField, VRMLNodeListener {
    ...
    private static LinkedList<SAIFieldEvent> eventList;
     ...
     */
    static {
        eventList = new LinkedList<SAIFieldEvent>();
    }
    ...
    public void fieldChanged(int index) {
        if(index == fieldIndex) {
            ...
            // Make sure we update our local state, and anyone that we depend
            // on right now, before calling
            updateField();
