...
public class SFRotation extends FieldScriptableObject {
     ...
     *
     * @param newAxis The new axis to use
     */
    public void jsFunction_setAxis(Scriptable newAxis) {
        if (!(newAxis instanceof SFVec3f)) {
            Context.reportRuntimeError(INVALID_TYPE_MSG);
        }
        SFVec3f axis = (SFVec3f) newAxis;
     ...
     *
     * @param newRotation The rotation to multiply this by
     * @return The result of the multiplication
     */
    public SFRotation jsFunction_multiply(Scriptable newRotation) {
        if (!(newRotation instanceof SFRotation)) {
            Context.reportRuntimeError(INVALID_TYPE_MSG);
        }

        // Retrieve the scripted rotation
        SFRotation rot = (SFRotation) newRotation;
        SFRotation ret_val = new SFRotation();

        // Create quaternions for multiplication operations
        Quat4d q1 = new Quat4d();
        Quat4d q2 = new Quat4d();

        // Establish the rotations to multiply
        AxisAngle4d aa1 = new AxisAngle4d(x,y,z,angle);
        AxisAngle4d aa2 = new AxisAngle4d(rot.x,rot.y,rot.z,rot.angle);

        // Place the rotations in the quaternions
        q1.set(aa1);
        q2.set(aa2);
        ...
        ret_val.y = aa1.getY();
        ret_val.z = aa1.getZ();
        ret_val.angle = aa1.getAngle();

        return ret_val;
    }

    /**
     * Turn this rotation into a rotation matrix and then multiply the vector
     * by that matrix. Mr . V = V' to give the return vector.
     *
     * @param newVector The vector to multiply by this rotation
     * @return A vector containing the result
     */
    public SFVec3f jsFunction_multVec(Scriptable newVector) {
