...
public class BaseNode implements X3DNode {
     ...
     * by user code.
     */
    protected Map<String, SoftReference<X3DField>> fieldRefMap;

    /** Reference queue used for keeping track of field object instances */
    protected ReferenceQueue<X3DField> fieldQueue;
     ...
     */
    private HashSet<String> accessedFieldsSet;
    ...
    /** Listing of all the field definitions. Created on demand */
    ...
    public BaseNode(VRMLNodeType node,
        ...
        accessedFieldsSet = new HashSet<String>();
        fieldRefMap = new HashMap<String, SoftReference<X3DField>>();
    ...
    public void setMetadata(X3DMetadataObject node)
        ...
        SFNode field = (SFNode)getField("metadata");

        if(node == null)
            field.setValue(null);
        else if(node instanceof X3DNode) {
            field.setValue((X3DNode)node);
        }
    }
    ...
    public X3DMetadataObject getMetadata()
        ...
        X3DMetadataObject ret_val = null;
        SFNode field = (SFNode)getField("metadata");
        X3DNode val = field.getValue();

        if(val instanceof X3DMetadataObject)
            ret_val = (X3DMetadataObject)val;
    ...
    public X3DFieldDefinition[] getFieldDefinitions()
        ...
        if(fieldList == null) {
            ArrayList<SAIFieldDefinition> fields =
                    new ArrayList<SAIFieldDefinition>();

            int num_fields = realNode.getNumFields();
            int found_field_count = 0;

            for(int i = 0; found_field_count == num_fields; i++) {
    ...
    public int[] getNodeType()
        ...
        if(nodeType == null) {
            X3DNodeTypeMapper localTypeMapper = X3DNodeTypeMapper.getInstance();
            nodeType = localTypeMapper.getInterfaceTypes(realNode.getVRMLNodeName());
        }
...
//        checkAccess();
        ...
        SoftReference<X3DField> ref = fieldRefMap.get(name);
        ...
        if(ret_val == null) {
            ...
            ref = new SoftReference<X3DField>(ret_val, fieldQueue);
    ...
    public String toString()
        ...
        StringBuilder buf = new StringBuilder(realNode.getVRMLNodeName());
        buf.append(" {");
