...
public class X3DExecutionContext extends AbstractScriptableObject {
    ...
    protected final static HashSet<String> propertyNames;

    /** Set of the valid function names for this object */
    protected final static HashSet<String> functionNames;
    ...
    protected List<SFNode> nodeWrapperList;

    /** Mapping of the VRMLNodeType to its X3DNode wrapper */
    protected Map<VRMLNodeType, SFNode> nodeWrapperMap;

    /** map containing imports info */
    protected Map<String, VRMLNode> importMap;
    ...
    /** Flag indicating that the contents of the rootNodes has changed */
    private boolean namedNodesChanged;
    ...
    public X3DExecutionContext(VRMLExecutionSpace space,
        ...
        namedNodesChanged = false;

        functionObjects = new HashMap<String, Object>();
        nodeWrapperList = new ArrayList<SFNode>();
        nodeWrapperMap = new HashMap<VRMLNodeType, SFNode>();
        ...
        if(space instanceof VRMLWorldRootNodeType) {
            ...
            rootNodes.setParentScope(X3DExecutionContext.this);
    ...
    public SFNode jsFunction_getNamedNode(String name) {
        ...
        SFNode ret_val = nodeWrapperMap.get(v_node);
...
errorReporter.messageReport("updateNamedNode() not implemented yet");
    ...
    public void jsFunction_removeNamedNode(String name) {
        ...
        // Look up to see if we have a cached wrapper of this node already.
        SFNode e_node = nodeWrapperMap.get(v_node);
...
errorReporter.messageReport("X3DExecutionContext does not implement updateImportedNode() yet");
    ...
    boolean hasSceneChanged() {
        ...
        // also work through the named node list and check to see if any have
        // been updated.
        namedNodesChanged = false;

        for(int i = 0; i < nodeWrapperList.size(); i++) {
            SFNode n = nodeWrapperList.get(i);
            namedNodesChanged |= n.hasChanged();
        }

        return ret_val || rootContentsChanged || namedNodesChanged;
    }
    ...
    public Object getChangedData() {
        ...
        if(namedNodesChanged) {
            List<NodeFieldData> l;

            if(ret_val instanceof List) {
                l = (List<NodeFieldData>)ret_val;
            } else if(ret_val instanceof NodeFieldData) {
                l = new ArrayList<NodeFieldData>();
                l.add((NodeFieldData)ret_val);
            ...
            for(int i = 0; i < nodeWrapperList.size(); i++) {
                ...
                Object changed = n.getChangedFields();

                if(changed != null) {
                    if(changed instanceof ArrayList) {
                       List<NodeFieldData> list =
                           (List<NodeFieldData>)changed;
                       l.addAll(list);
                    } else {
                        l.add((NodeFieldData)changed);
                    }
                }
            }
    ...
    /**
     * Convenience method to regenerate the RouteArray object so that
     * the user can peruse the list of routes in this method call.
     *
     * @return A freshly generated set of routes
     */
    private RouteArray generateRoutes() {
        ...
        return  new RouteArray(route_list);
    }
