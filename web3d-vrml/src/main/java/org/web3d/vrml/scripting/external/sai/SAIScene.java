...
class SAIScene implements X3DScene {
     ...
     * @param aFactory The factory to map between SAINode instances and
     *    VRMLNodeType instances
    ...
    public X3DNode getExportedNode(String name) {
        VRMLNodeType export=(VRMLNodeType)(theScene.getDEFNodes().get(name));
        if (export==null)
    ...
    public String[] getExportedNodes() {
        return theScene.getExports().keySet().toArray(EMPTY_STRING_ARRAY);
    }
    ...
    public ProfileInfo getProfile() {
        ...
        org.web3d.vrml.lang.ProfileInfo[] profiles=
            theScene.getNodeFactory().getAvailableProfiles();
        for (org.web3d.vrml.lang.ProfileInfo profile : profiles) {
            if (profile.getName().equals(profileName)) {
                return new SAIProfileInfo(profile);
            }
        }
    ...
    public ComponentInfo[] getComponents() {
        org.web3d.vrml.lang.ComponentInfo[] components=
            theScene.getMetaData().getComponents();
    ...
    public String[] getProtosNames() {
        return theScene.getProtos().keySet().toArray(EMPTY_STRING_ARRAY);
    }
    ...
    public X3DProtoDeclaration getProtoDeclaration(String name) {
        Map<String, VRMLProtoDeclare> proto_map = theScene.getProtos();
        VRMLProtoDeclare decl = proto_map.get(name);
    ...
    public String[] getExternProtoNames() {
        return theScene.getExternProtos().keySet().toArray(EMPTY_STRING_ARRAY);
    }
    ...
    public String[] getNamedNodes() {
        Set<String> keys=theScene.getDEFNodes().keySet();
