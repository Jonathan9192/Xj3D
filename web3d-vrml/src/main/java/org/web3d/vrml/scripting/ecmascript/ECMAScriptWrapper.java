...
class ECMAScriptWrapper implements ScriptWrapper {
     ...
     */
    private static final HashSet<String> NON_USE_FIELDS;
     ...
     */
    static {
        NON_USE_FIELDS = new HashSet<String>();
     ...
     * @param sc The script (as a string) that this class is wrapping
    ...
    public void initialize(VRMLScriptNodeType node) {
        ...
        eventOutMap = new HashMap<String, Object>();
        nodeEventOuts = new HashMap<String, NodeFieldObject>();

        // Find all of the fields declared in the node and create representative
        // field instances of them.
        List<VRMLFieldDeclaration> field_list = node.getAllFields();
        Iterator<VRMLFieldDeclaration> itr = field_list.iterator();

        List<String> eo_names = new ArrayList<String>(field_list.size() - 3);
        ...
        while(itr.hasNext()) {
            ...
            // don't define eventIn fields as objects in the JavascriptScript space.
            ...
            try {
                ...
                switch(decl.getFieldType()) {
                    ...
                    case FieldConstants.SFNODE:
                        ...
                        nodeEventOuts.put(name, (SFNode)field_wrapper);
                        break;
                    ...
                    case FieldConstants.MFDOUBLE:
                        d_arg = (data == null) ? null : data.doubleArrayValue;
                        field_wrapper = new MFDouble(d_arg, data.numElements);
                        break;
                    ...
                    case FieldConstants.MFNODE:
                        ...
                        nodeEventOuts.put(name, (MFNode)field_wrapper);
                        break;
        ...
        if(directOutput) {
            ...
            Set<String> keys = nodeEventOuts.keySet();
...
//                    eventOutMap.put(name, functionArgs[0]);
    ...
    private boolean sendNodeOutput(String fieldName, boolean changeChecked) {
        NodeFieldObject nfo = nodeEventOuts.get(fieldName);
        ...
        if(changed_items instanceof NodeFieldData) {
            ...
            List<NodeFieldData> list =
               (List<NodeFieldData>)changed_items;
    ...
    private void sendSceneOutput() {
        ...
        if(changed_items instanceof NodeFieldData) {
            ...
            // must be an array list of stuff
            List<NodeFieldData> list = (List<NodeFieldData>)changed_items;

            int i_size = list.size();
            for(int j = 0; j < i_size; j++) {
                NodeFieldData data = list.get(j);
                try {
                    sendEvent(data.node, data.fieldIndex, data.value);
                } catch(FieldException fe) {
                    Context.reportWarning(fe.getMessage());
