 ...
 */
public class SFVec4d extends FieldScriptableObject {
    ...
    /**
     * Alternate constructor that takes a double array rather than a double
     * array. Used primarily by the VrmlMatrix code.
     */
    public SFVec4d(double[] vec) {
        this();
     ...
     */
    public SFVec4d jsFunction_add(Scriptable newVector) {
        if (!(newVector instanceof SFVec4d)) {
            Context.reportRuntimeError(INVALID_TYPE_MSG);
        }

        SFVec4d vec = (SFVec4d) newVector;
        SFVec4d ret_val = new SFVec4d();
     ...
     * @return The product
     */
    public SFVec4d jsFunction_cross(Scriptable newVector) {
        if (!(newVector instanceof SFVec4d)) {
            Context.reportRuntimeError(INVALID_TYPE_MSG);
        }
     ...
     * @return The addition of the two objects
     */
    public SFVec4d jsFunction_divide(double n) {
        SFVec4d ret_val = new SFVec4d();
        ret_val.x = x / n;
        ret_val.y = y / n;
        ret_val.z = z / n;
     ...
     * @return The addition of the two objects
     */
    public SFVec4d jsFunction_multiply(double n) {
        SFVec4d ret_val = new SFVec4d();
        ret_val.x = x * n;
        ret_val.y = y * n;
        ret_val.z = z * n;
     ...
     */
    public SFVec4d jsFunction_negate() {
        SFVec4d ret_val = new SFVec4d();
        ret_val.x = -x;
        ret_val.y = -y;
        ret_val.z = -z;
     ...
     */
    public SFVec4d jsFunction_normalize() {
        SFVec4d ret_val = new SFVec4d();
     ...
     */
    public SFVec4d jsFunction_subtract(Scriptable newVector) {
        if (!(newVector instanceof SFVec4d)) {
            Context.reportRuntimeError(INVALID_TYPE_MSG);
        }
    ...
    public boolean equals(Object val) {
        if(!(val instanceof SFVec4d))
            return false;

        SFVec4d o = (SFVec4d)val;
     ...
     * provided array as double values.
     *
