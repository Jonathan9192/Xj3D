...
public class MFNode extends NodeFieldObject {
        ...
        if(numValid != 0) {
            ...
            for(int i = 0; i < numValid; i++) {
                if(nodes[i] != null) {
                    ...
                    node.setParentScope(MFNode.this);
                    nodeList.add(node);
                } else
                    nodeList.add(null);
            }
    ...
    public MFNode(Object[] args) {
        ...
        for(int i=0; i < args.length; i++) {
            ...
            if(a_node.getParentScope() == null)
                a_node.setParentScope(MFNode.this);
    ...
    public void put(int index, Scriptable start, Object value) {
        ...
        if(!changedNodes.contains((SFNode)value))
    ...
    public Object getChangedFields() {
        ...
        if(size == 1) {
            SFNode node = changedNodes.get(0);
            if(node != null)
                ret_val = node.getChangedFields();
        } else {
            for(int i = 0; i < size; i++) {
                SFNode node = changedNodes.get(i);
                Object values = null;
