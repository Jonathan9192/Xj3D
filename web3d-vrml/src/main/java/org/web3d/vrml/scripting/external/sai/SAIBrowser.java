...
//        externalNavManager.shutdown();
    ...
    public ComponentInfo getComponent(String name, int level)
        ...
        for(int i=0; i<components.length; i++) {
            if(components[i].getName().equals(name)) {
                if(components[i].getLevel()>=level) {
                    retVal = new SAIComponentInfo(name,
                                                  level,
                                                  components[i].getTitle(),
                                                  components[i].getProviderURL());
                    break;
                } else
    ...
    public ProfileInfo getProfile(String name) {
        ...
        for(int counter=0; counter<profiles.length; counter++) {
            if(profiles[counter].getName().equals(name)) {
                retVal = profiles[counter];
                break;
             }
        }
    ...
    public void linkActivated(String[] urls, String[] params, String desc) {
        ...
        for(int i = 0; i < urls.length; i++) {
            if(urls[i].charAt(0) == '#') {
                ...
                String def_name = urls[i].substring(1);
