...
public class NetworkEventQueue implements ExternalView {
    ...
    private Map<ExternalEvent, ExternalEvent> queuedEvents;
    ...
    List<ExternalEvent> waitingQueue;
    ...
    public NetworkEventQueue(ErrorReporter reporter) {
        ...
        eventQueue=new LinkedList<ExternalEvent>();
        queuedEvents=new HashMap<ExternalEvent, ExternalEvent>();
        waitingQueue=new LinkedList<ExternalEvent>();
        if (reporter==null)
    ...
    /** End queuing of updates, and send the current set to be processed.
      * It isn't an error to call this multiple
      * times, but extra calls are ignored.
      */
    ...
    public void processEvents() {
        ...
        List<ExternalEvent> queue;
        synchronized (eventLock) {
            queue=waitingQueue;
            waitingQueue=new LinkedList<ExternalEvent>();
        }
        while (!queue.isEmpty()) {
            ExternalEvent event=((LinkedList<ExternalEvent>)queue).removeFirst();
            try {
                event.doEvent();
            } catch (Exception e) {
