...
public class Browser extends AbstractScriptableObject {
    ...
    private final static HashSet<String> functionNames;
    ...
    public Browser(VRMLExecutionSpace space,
        ...
        nameToProfileMap = new HashMap<String, ProfileInfo>();
        stateManager = fsm;
        routeManager = rm;
        ...
        // Set up the profile list and this profile definition
        ...
        for(int i = 0; i < p_list.length; i++) {
            ProfileInfo p_info = new ProfileInfo(p_list[i]);
        ...
        ecmaScene.setParentScope(Browser.this);
    }
            ...
            switch(prop) {
                ...
                case 's':
                    if(name.equals("supportedProfiles"))
                        ret_val = generateProfiles();
                    else
                        ret_val = generateComponents();
     ...
     *
     * @param scene The list of nodes to use as the new root of the world
     */
    ...
    /**
     * Generate the component listing array again from the end user
     * call.
     *
     * @return A freshly generated component array
     */
    private ComponentInfoArray generateComponents() {
        VRMLNodeFactory node_fac = internalScene.getNodeFactory();
        ...
        return new ComponentInfoArray(ecma_comps);
    }

    /**
     * Regenerate the array of profiles available to the system.
     *
     * @return A freshly generated profile array
     */
    private ProfileInfoArray generateProfiles() {
        VRMLNodeFactory node_fac = internalScene.getNodeFactory();
        ...
        for(int i = 0; i < p_list.length; i++) {
            ...
            nameToProfileMap.put(p_list[i].getName(), ecma_profiles[i]);
        }

        return new ProfileInfoArray(ecma_profiles);
    }
