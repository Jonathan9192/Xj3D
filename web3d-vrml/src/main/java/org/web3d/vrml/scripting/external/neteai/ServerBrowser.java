...
public class ServerBrowser {
    ...
    /** Table for mapping VRMLNodeType objects to integer node ID's.  Should not be
        ...
        * is closed.  */
    Map<VRMLNode, Integer> nodeToNodeIDTable;
    ...
    public ServerBrowser(BrowserCore browserCore, VRML97CommonBrowser vrmlBrowser) {
        ...
        nodeToNodeIDTable=new HashMap<VRMLNode, Integer>();
    ...
    public int generateNodeID(VRMLNodeType node) {
        Integer oldID=nodeToNodeIDTable.get(node);
        if (oldID!=null)
    ...
    /** Pass loadURL request off to browser impl.
    ...
    /** Register the field changed transmitter.  Needed to separate from the
    ...
    /** Register the browser core listener.  Needed separate from
     * constructor to avoid constructor loop.
