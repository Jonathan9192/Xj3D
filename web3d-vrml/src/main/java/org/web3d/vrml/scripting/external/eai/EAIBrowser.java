...
public class EAIBrowser implements Browser, BrowserCoreListener {
    ...
    public EAIBrowser(BrowserCore browserCore,
        ...
        browserListeners=new Vector<BrowserListener>();
        browserCore.addCoreListener(EAIBrowser.this);
    ...
    public Node getNode(String nodeName)
        ...
        if (theBrowserImpl==null)
            ...
            Map<String, VRMLNode> map = theBrowserCore.getDEFMappings();
            System.out.println("DEF Map Size: " + map.size());

            VRMLNodeType who=(VRMLNodeType) map.get(nodeName);
            if (who!=null)
    ...
    void broadcastEvent(BrowserEvent e) {
        Enumeration<BrowserListener> en=browserListeners.elements();
        while (en.hasMoreElements()) {
            BrowserListener b=en.nextElement();
...
*/
    ...
    /** Convenience class for the security actions.
      * Just creates arbitrary threads using the passed in Runnable's.
      *  */
    class CreateThreadAction implements PrivilegedAction<Thread> {
