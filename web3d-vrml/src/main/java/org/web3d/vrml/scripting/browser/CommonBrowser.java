...
public abstract class CommonBrowser {
    ...
    /** Message when the user's home directory as a URL fails */
    private static final String BAD_BASE_URL_MSG =
        "The user's default directory cannot be created as a URL. I sense a " +
        "terrible disturbance in the force.";
    ...
    protected String[] completeUrl(String[] base) {
        ...
        if (worldURL == null) {
            try {
                ...
                System.err.println("Error getting user.dir");
            }
            ...
            // Linux and Mac already start with a leading / so don't add too
            // many on there.
            if(url_base.startsWith("/"))
                complete_url = "file://" + url_base;
            else
                complete_url = "file:///" + url_base;
