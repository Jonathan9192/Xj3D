 ...
 */
public class MFColorRGBA extends FieldScriptableObject {

    private static final String OBJECT_NOT_COLOR_MSG =
        "The object you attempted to assign was not an SFColorRGBA instance";

    /** The properties of this class */
    private ArrayList<SFColorRGBA> valueList;
    ...
    public MFColorRGBA() {
        ...
        valueList = new ArrayList<SFColorRGBA>();
     ...
     * node).
     */
    public MFColorRGBA(float[] values, int numValid) {
        this();

        workArray = new float[4];
     ...
     */
    public MFColorRGBA(Object[] args) {
        this();
        ...
        for(int i=0; i < args.length; i++) {
            ...
            if(!(args[i] instanceof SFColorRGBA))
                    throw new IllegalArgumentException("Non SFColorRGBA given");

            cnt++;
            valueList.add((SFColorRGBA) args[i]);
        }
    ...
    public static Scriptable jsConstructor(Context cx, Object[] args,
        ...
        MFColorRGBA result = new MFColorRGBA(args);
    ...
    public Object get(int index, Scriptable start) {
        ...
        if((index >= 0) && (index < valueList.size())) {
            ...
            if(ret_val == null) {
                ...
                SFColorRGBA n = new SFColorRGBA(workArray);
    ...
    public void put(int index, Scriptable start, Object value) {
        ...
        Scriptable node = (SFColorRGBA)value;
        ...
        if(index >= valueList.size()) {
            ...
            for(int i=0; i < toAdd; i++) {
                valueList.add(new SFColorRGBA());
            }

            valueList.add((SFColorRGBA) value);
            sizeInt.setValue(valueList.size());
        } else if(index >= 0) {
            valueList.set(index, (SFColorRGBA) value);
        }
    ...
    public String toString() {
        ...
        for(int i = 0; i < size; i++) {
            SFColorRGBA node = valueList.get(i);
    ...
    public boolean equals(Object val) {
        if(!(val instanceof MFColorRGBA))
            return false;

        MFColorRGBA o = (MFColorRGBA)val;
     ...
     *
     * @return A flat array of values [r1, g1, b1, a1, r2, g2, b2, a2,....]
     */
    public float[] getRawData() {
        int size = valueList.size();

        if((floatData == null) || (floatData.length != (size * 4)))
            floatData = new float[size * 4];
