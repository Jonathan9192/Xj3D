...
public class SFVec4f extends FieldScriptableObject {
     ...
     */
    public SFVec4f jsFunction_add(Scriptable newVector) {
        if (!(newVector instanceof SFVec4f)) {
            Context.reportRuntimeError(INVALID_TYPE_MSG);
        }

        SFVec4f vec = (SFVec4f) newVector;
     ...
     * @return The product
     */
    public SFVec4f jsFunction_cross(Scriptable newVector) {
        if (!(newVector instanceof SFVec4f)) {
            Context.reportRuntimeError(INVALID_TYPE_MSG);
        }
     ...
     */
    public SFVec4f jsFunction_subtract(Scriptable newVector) {
        if (!(newVector instanceof SFVec4f)) {
            Context.reportRuntimeError(INVALID_TYPE_MSG);
        }
