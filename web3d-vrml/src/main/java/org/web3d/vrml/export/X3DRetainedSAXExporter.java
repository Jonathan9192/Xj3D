...
public abstract class X3DRetainedSAXExporter extends BaseRetainedExporter
    ...
    protected HashSet<VRMLNodeType> usedNodes;
    ...
    protected HashSet<PrototypeDecl> protoDeclSet;
    ...
    private void init() {
        usedNodes = new HashSet<VRMLNodeType>();
        indentString = "";
        indentMap = new IntHashMap();
        protoDeclSet = new HashSet<PrototypeDecl>();
        traverser = new SceneGraphTraverser();
        protoMap = new HashMap<String, VRMLFieldDeclaration>();
    ...
    public void setX3DAttributes(Map<String,String> atts) {
        if (atts != null) {
            x3dAtts = new HashMap<String,String>();
            x3dAtts.putAll(atts);
        } else {
            x3dAtts = null;
        }
        ...
        if (ismf) {
            ...
            for(int j=0; j < len2; j++) {
                for(int k=0; k < span; k++) {
                    if (sigDigits < 0)
                        ...
                        DoubleToString.appendFormatted(buff, (double) val[idx++], sigDigits);
                    }
                    if (k != span - 1)
                        buff.append(" ");
                }
            ...
            for(int j=0; j < len2; j++) {
                if (sigDigits < 0)
                    ...
                    DoubleToString.appendFormatted(buff, (double) val[j], sigDigits);
                }
    ...
    public void writeScene(VRMLScene scene) {
        ...
        currentDefMap = new HashMap<VRMLNode, String>(defs.size());
        ...
        while(itr.hasNext()) {
            ...
            if (proto instanceof ExternalPrototypeDecl) {
                ...
                List<VRMLFieldDeclaration> fields = ((AbstractProto)proto).getAllFields();
    ...
    public void printROUTE(ROUTE route, Map<VRMLNode, String> defMap) {
        ...
        String sourceDEF = defMap.get(source);
        String destDEF;
...
*/
    ...
    public boolean printStartNode(VRMLNodeType node, boolean use, Map<VRMLNode, String> defMap,
        ...
        List<VRMLFieldDeclaration> fields = new ArrayList<VRMLFieldDeclaration>();

        if (node instanceof AbstractProto) {
            List<VRMLFieldDeclaration> pfields = ((AbstractProto)node).getAllFields();
...
*/
    ...
    public void printPrototypeDecl(PrototypeDecl proto) {
        currentDefMap = new HashMap<VRMLNode, String>();
        Map<VRMLNode, String> saveMap = currentDefMap;
...
//System.out.println("RetainedSaxExporter " + attName + " " +  java.util.Arrays.toString(data.intArrayValue));
                    atts.addAttributeWithAlgorithmData(
            ...
            case VRMLFieldData.STRING_ARRAY_DATA:
                ...
                for(int j=0; j < len2; j++) {
                    if (j == len2 -1) {
                        sbuff.append("\"");
                        if (binary && false) {
System.out.println("Not escaping2: " + data.stringArrayValue[j]);

                            sbuff.append(data.stringArrayValue[j]);
                        } else
                        ...
                        sbuff.append("\"");
                        if (binary && false) {
System.out.println("Not escaping3: " + data.stringArrayValue[j]);
                            sbuff.append(data.stringArrayValue[j]);
                        } else
                            sbuff.append(XMLTools.XML.escape(data.stringArrayValue[j]));
