...
public class CompressionTools {
    ...
    private static byte[] temp_result = new byte[1024];
...
//System.out.println("Num Bits: " + numBits + " exp: " + exponent + " man: " + mantissa + " len: " + fval.length + " size: " + (fval.length * numBits / 8f));
        ...
        BitPacker packer = new BitPacker((int)Math.ceil(len * numBits / 8.0f) + 1);
...
*/
    ...
    public static int calcRange(boolean delta, int span, int[] ival) {
        ...
        int scnt=0;
        if (delta) {
            System.arraycopy(ival, 0, lastVal, 0, span);
            int hold;
...
//System.out.println("bytes:  len: " + vals.length + " numBits: " + numBits + " maxBytes: " + Math.ceil((vals.length * numBits / 8f)));
        ...
        BitPacker packer = new BitPacker((int)Math.ceil(vals.length * numBits / 8f) + 1);
...
//System.out.println("Num Bits: " + numBits + " exp: " + exponent + " man: " + mantissa + " len: " + fval.length + " size: " + (fval.length * numBits / 8f));
        ...
        BitPacker packer = new BitPacker((int)Math.ceil(len * numBits / 8f) + 1);
        int data;
        for(int i=0; i < len; i++) {
...
*/
    ...
    /**
     * Compress a float array by quantizing its values.
     *
     * @param dos The output stream
     * @param fval The float values
     * @param tolerance The tolerance allowed between equal values
     */
    public static void quantizeFloatArrayDeflater(DataOutputStream dos, float fval[],
        float tolerance, boolean useLen) throws IOException {
...
//System.out.println("Incoming float size: " + fval.length + " useLen: " + useLen + " tolerance: " + tolerance);
...
//System.out.println("exponent: " + exponent + " mantissa: " + mantissa);
        FloatPacker encoder = new FloatPacker(exponent, mantissa);
        int bits;
        int len = fval.length;

        if (useLen) {
            // Old impl always wrote 0 and was ignored
            dos.writeByte(exponent);
            dos.writeByte(mantissa);
        } else {
            BitPacker bp = new BitPacker(8);

            bp.pack(1, 1);  // TODO: change when we support unsigned value space saving
            bp.pack(exponent, 7);
        ...
        int max_size = (int)Math.ceil(len * numBits / 8f) + 1;

        BitPacker packer = new BitPacker(max_size);
        int data;
        float test;
...
//System.out.println("Before deflater: " + vals.length);
        ...
        if (useLen) {
            dos.writeInt(vals.length);   // Rounded up size derived from bits, could be calced?
        }

        dos.writeInt(len);           // Number of floats in stream
        ...
        while(more) {
            ...
            tot_written += compressedDataLength;
        }

//System.out.println("compress length: " + tot_written);
        // TODO: Deflaters must be explicity ended to avoid native memory loss
        compresser.end();
    }

    public static float[] dequantizeFloatArrayInflater(byte[] data, int start, int length,
        boolean useLen) throws IOException {

        byte exponent = data[start++];
        byte mantissa = data[start++];


        // Strip out sign needed bit:
        boolean signed;
        ...
        exponent = (byte) (127 & exponent);

        int len = 0;

        if (useLen) {
            len = readInt(data, start);
            start += 4;
        }
        ...
        if (!useLen) {
            len = (int) Math.ceil(numFloats * numBits / 8f);
        }
...
//System.out.println("len: " + len + " res_len: " + temp_result.length);
        try {
            Inflater decompresser = new Inflater(false);
            decompresser.setInput(data, start, length - 10);
            //decompresser.setInput(data, start, length - 6);  // XIOT has -6 why?

            // Suggestion is this could be numFloats * 4.
            // Could also look at reusing a buffer to avoid so much allocation during parsing.
            // When would you shrink this buffer as it could get rather large for big data
            int resultLength = decompresser.inflate(temp_result);
            decompresser.end();

        } catch(Exception e) {
            System.err.println("Invalid format in dequantizeFloatArrayInflater");
            e.printStackTrace();
        }
...
//System.out.println();
            ...
            for(int i=0; i < len; i++) {
                node = new ByteHuffmanNode(delts[i]);
                deltaTable.addEntry(node);
            }
            deltaTable.computeTags();
...
//int tsize = dos.size();
        ...
        BitPacker bp = new BitPacker((int)Math.ceil(ival.length * bits / 8.0) + 1);
        int val;
        for(int i=0; i < ival.length; i++) {
            val = ival[i] + shift;
            bp.pack(val, bits);
...
System.out.println("final mantissa: " + mantissa);
     ...
     * express the float value within the tolerance specified.
     *
     * @param origval The original value
     * @param tolerance The error tolerance
     * @param output The exponent, mantissa and sign bits needed
     */
...
if (ldebug) System.out.println("exponent bits: " + exponent);
        ...
        if (searchup) {
            ...
            output[1] = mantissa;
            output[2] = signed;
if (ldebug) System.out.println("final params1: exp: " + exponent + " mantissa: " + mantissa + " signed: " + signed);
...
if (ldebug) System.out.println("final params2: exp: " + exponent + " mantissa: 0" + " signed: " + signed);
            return;
        }
...
if (ldebug) System.out.println("final params: exp: " + exponent + " mantissa: " + mantissa + " signed: " + signed);

if ((exponent > exponent_initial + 1) || (mantissa > mantissa_initial + 1)) {
    ...
    /**
     * Find the float parameters of exponent, mantissa and sign that will
     * express the array of float values within the tolerance specified.
     *
    ...
    public static void findFloatParams(float[] origvals, float tolerance, int[] output) {
        ...
        //System.out.println("Params: " + java.util.Arrays.toString(output));
    }
    ...
    private static void testAllFloatsSize(int size, float start, int step, float tolerance) {
        ...
        try {
            quantizeFloatArrayDeflater(dos, array, tolerance, false);
