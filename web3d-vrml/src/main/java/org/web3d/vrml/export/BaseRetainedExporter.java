 ...
 *    ExternProto URL already has worldUrl added in, removing worldRoot might not work
 *
 * TODO:
...
public abstract class BaseRetainedExporter extends Exporter implements BinaryContentHandler {
    ...
    protected Map<String, VRMLNode> defaultNodes;
    ...
    protected Map<VRMLNode, String> currentDefMap;

    /** The current IS map */
    protected Map<Integer, List<ProtoFieldInfo>> currentIsMap;
    ...
    protected Map<String, String> oldProtos;

    /** Fields to remap their types.  Node.fieldName -> new type */
    protected Map<String, Integer> fieldRemap;

    /** Where there H-Anim nodes found */
    protected boolean hanimFound;

    /** H-Anim nodes */
    protected Map<String, String> hanimNodes;

    /** Where there Geospatial nodes found */
    protected boolean geospatialFound;

    /** Geospatial nodes */
    protected Map<String, String> geospatialNodes;

    /** A cache of IS map preprocessed versions */
    protected Map<Map<Integer, List<ProtoFieldInfo>>, Map.Entry<Integer, List<ProtoFieldInfo>>[]> isCache;
    ...
    public BaseRetainedExporter(int major,
        ...
        defaultNodes = new HashMap<String, VRMLNode>();
        componentList = new ArrayList<ComponentInfo>();
        isCache = new HashMap<Map<Integer, List<ProtoFieldInfo>>, Map.Entry<Integer, List<ProtoFieldInfo>>[]>();

        currentDefTable = new HashMap<String, String>();
        currentDefRemapTable = new HashMap<String, String>();
        currentDefNumTable = new HashMap<String, Integer>();

        defStack = new SimpleStack();
        defRemapStack = new SimpleStack();
        defNumStack = new SimpleStack();

        epToUrl = new HashMap<String, String[]>();
     ...
     */
    protected abstract void printMetaData(Map<String, String> meta);
     ...
     */
    protected abstract void printROUTE(ROUTE route, Map<VRMLNode, String> defMap);
     ...
     */
    protected abstract void printExports(Map<String, String> exports);
     ...
     */
    protected abstract void printImports(Map<String, VRMLNode> imports);
    ...
    public void endDocument() throws SAVException, VRMLException {
        ...
        if (baseSize > 0) {
            System.arraycopy(baseComps, 0, comps, start, baseSize);
        }
    ...
    public void startNode(String name, String defName)
        ...
        if (defName != null) {
            if (currentDefTable.get(defName) != null) {
                ...
                while(currentDefTable.get(newName) != null) {
                    num = currentDefNumTable.get(defName);
                    ...
                    newName = defName + "_DUP" + num.intValue();
                }
    ...
    public void useDecl(String defName)
        ...
        String newName = currentDefRemapTable.get(defName);
        if (newName != null)
            defName = newName;
    ...
    public void endProtoBody() throws SAVException, VRMLException{
        ...
        currentDefTable = (Map<String, String>) defStack.pop();
        currentDefRemapTable = (Map<String, String>) defRemapStack.pop();
        currentDefNumTable = (Map<String, Integer>) defNumStack.pop();
    }
     ...
     * @return the IS relationship for a node
     */
    protected String findIS(VRMLNodeType node, int idx, Map<Integer, List<ProtoFieldInfo>> isMap) {
        if (isMap == null)
            return null;

        Map.Entry<Integer, List<ProtoFieldInfo>>[] map = isCache.get(isMap);
        ProtoFieldInfo pfi;
        List<ProtoFieldInfo> list;
        ...
        for (Entry<Integer, List<ProtoFieldInfo>> map1 : map) {
            list = map1.getValue();
            for (int j = 0; j < list.size(); j++) {
                pfi = list.get(j);
                if (pfi.node == node && pfi.field == idx) {
                    VRMLFieldDeclaration decl = currentPrototypeDecl.getFieldDeclaration(map1.getKey().intValue());
                    return decl.getName();
                }
            }
        }
     ...
     */
    protected void printIS(Map<Integer, List<ProtoFieldInfo>> isMap) {
        if (isMap == null)
            return;

        List<ProtoFieldInfo> list;
        Map.Entry<Integer, List<ProtoFieldInfo>>[] map = new Map.Entry[isMap.size()];
        ProtoFieldInfo pfi;
     ...
     */
    protected void printDefMap(Map<VRMLNode, String> defMap) {
        Map.Entry[] entries;
        ...
        for (Entry entrie : entries) {
            errorReporter.messageReport("   " + entrie.getKey() + " --> " + entrie.getValue());
        }
    }
     ...
     * @param in map to reverse
     * @param out reversed map
     */
    protected void reverseMap(Map<String, VRMLNode> in, Map<VRMLNode, String> out) {
        Map.Entry<String, VRMLNode> entry;
        Set<Entry<String, VRMLNode>> entrySet = in.entrySet();
        Iterator<Entry<String, VRMLNode>> itr = entrySet.iterator();
        VRMLNode node;
        String defName;
        ...
        while(itr.hasNext()) {
            entry = itr.next();
            node = entry.getValue();
            defName = entry.getKey();

            out.put(node, defName);
        }
    }
    ...
    private void initOldProtos() {
        fieldRemap = new HashMap<String, Integer>();

        hanimNodes = new HashMap<String, String>();
        ...
        geospatialNodes = new HashMap<String, String>();
        ...
        oldProtos = new HashMap<String, String>(hanimNodes.size() + geospatialNodes.size());
        ...
        switch(newType) {
            case VRMLFieldData.DOUBLE_DATA:
                switch(data.dataType) {
                    ...
                    default:
                        StringBuilder buf =
                            new StringBuilder("Unhandled type conversion in convertFieldData:");
                        buf.append(decl);
                        buf.append("\noriginal type: ");
