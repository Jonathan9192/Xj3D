 ...
 */
public class OrientationInterpolatorCompressor implements NodeCompressor {
    ...
    public void fieldValue(String value) throws SAVException, VRMLException {
        ...
        try {
            if (currentField == 0) {
                ...
                int dbits = CompressionTools.calcRange(true, 4, ival);
                int rbits = CompressionTools.calcRange(false, 4, ival);

                if (dbits < rbits) {
                    bp.pack(0,1);
                    CompressionTools.encodeFloatArray(dos, bp, true, 4, fval);
                } else {
                    bp.pack(0,1);
                    CompressionTools.encodeFloatArray(dos, bp, false, 4, fval);
                }
            }
