...
/**
 * A node compressor for TextureCoordinate.  Simple delta encoding.
 *
 * @author Alan Hudson.
 * @version $Revision: 1.3 $
 */
public class TextureCoordinateCompressor implements NodeCompressor {
    ...
    public void fieldValue(String value) throws SAVException, VRMLException {
        ...
        float[] fval = fieldParser.MFVec2f(value);

        try {
            // It doesn't seem that delta encoding helps texture coordiantes much
            CompressionTools.encodeFloatArray(dos, null, false, 2, fval);

            /*
            int[] ival = new int[fval.length];

            int mult = CompressionTools.convertFloatArrayToIntArray(fval, ival);
            int dbits = CompressionTools.calcRange(true, 2, ival);
            int rbits = CompressionTools.calcRange(false, 2, ival);

            BitPacker bp = new BitPacker(9);

            if (dbits < rbits) {
                bp.pack(0,1);
                CompressionTools.encodeFloatArray(dos, bp, true, 2, fval);
            } else {
                bp.pack(1,1);
                CompressionTools.encodeFloatArray(dos, bp, false, 2, fval);
            }
            */
