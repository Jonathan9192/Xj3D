...
/**
 * A node compressor for PositionInterpolator nodes.  Uses delta encoding on the
 * key field as most are small increments.
 *
 * TODO: Look at doing predictive + delta encoding
 *
 * @author Alan Hudson.
 * @version $Revision: 1.4 $
 */
public class PositionInterpolatorCompressor implements NodeCompressor {
    ...
    public void fieldValue(String value) throws SAVException, VRMLException {
        ...
        BitPacker bp = new BitPacker(9);

        bp.pack(currentField,1);

        try {
            if (currentField == 0) {
                fval = fieldParser.MFFloat(value);
                System.out.println("key  len: " + fval.length);
                // Always delta encode these
                CompressionTools.encodeFloatArray(dos,bp,true,1,fval);
            } else {
                fval = fieldParser.MFVec3f(value);

                int[] ival = new int[fval.length];
                int mult = CompressionTools.convertFloatArrayToIntArray(fval, ival);
                int dbits = CompressionTools.calcRange(true, 3, ival);
                int rbits = CompressionTools.calcRange(false, 3, ival);

                if (dbits < rbits) {
                    bp.pack(0,1);
                    CompressionTools.encodeFloatArray(dos, bp, true, 3, fval);
                } else {
                    bp.pack(0,1);
                    CompressionTools.encodeFloatArray(dos, bp, false, 3, fval);
                }
            }
