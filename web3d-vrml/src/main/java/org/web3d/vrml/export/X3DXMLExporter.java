 ...
 *     a stack of StringBuilders where IS information is kept per node.
 *         push on startNode
...
public class X3DXMLExporter extends Exporter {
    ...
    private Map<String, String> defTable;
    ...
    /** String representation of an indent w/spaces */
    ...
    /** The current nodes attributes as a StringBuilder */
    private SimpleStack attribs;

    /** A buffer containing the current nodes attributes */
    private StringBuilder currentAttribs;

    /** The current node's SFNode/MFNode children as a StringBuilder */
    private SimpleStack nodes;

    /** A buffer containing the current nodes nodes */
    private StringBuilder currentNodes;
    ...
    public X3DXMLExporter(OutputStream os,
        ...
        nodeMap = new HashMap<String, VRMLNode>(50);
        protoMap = new HashMap<String, PrototypeDecl>();
        externProtoMap = new HashMap<String, ExternalPrototypeDecl>();
        defTable = new HashMap<String, String>();
        defRemapTable = new HashMap<String, String>();
        defNumTable = new HashMap<String, Integer>();
    }
    ...
    public void startNode(String name, String defName)
        ...
        currentNodes = new StringBuilder();
        currentNodeName = name;
        currentAttribs = new StringBuilder();
        nlevel++;
        ...
        if (node == null) {
            if(protoMap.containsKey(name) || externProtoMap.containsKey(name)) {
                ...
                if(proto_def == null) {
                    ExternalPrototypeDecl eproto_def = externProtoMap.get(name);
        ...
        currentIS.push(new StringBuilder());
    }
    ...
    public void endNode()
        ...
        currentAttribs = (StringBuilder) attribs.pop();
        currentNodes = (StringBuilder) nodes.pop();
        ...
        StringBuilder cis = (StringBuilder) currentIS.pop();
        ...
        if (!nodes.isEmpty())
            parent = (StringBuilder) nodes.pop();
    ...
    public void startField(String name)
        ...
        if (type > 1) {
            ...
            if (name.equals("url") && ((VRMLNodeType)node).getVRMLNodeName().equals("Script")) {
                inCData=true;
                currentNodes = (StringBuilder) nodes.peek();
                currentAttribs = (StringBuilder) attribs.peek();
            } else {
                inCData=false;
                currentNodes = (StringBuilder) nodes.peek();
                currentAttribs = (StringBuilder) attribs.peek();
    ...
    public void useDecl(String defName)
        ...
        currentNodes.append("<");
        String defType = defTable.get(defName);
    ...
    public void endProtoDecl()
        ...
        StringBuilder parent = (StringBuilder) nodes.pop();
    ...
    public void protoIsDecl(String fieldName)
        ...
        if (lastStartField) {
            StringBuilder cis = (StringBuilder) currentIS.peek();
