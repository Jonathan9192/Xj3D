...
public class HuffmanTable {
    ...
    private Map<HuffmanNode, HuffmanNode> values;
    ...
    /**
     * Create a new HuffmanTable with entries for all possible position, normal,
     * and color tokens.
     */
    HuffmanTable() {
        values = new HashMap<HuffmanNode, HuffmanNode>();
        dataLength = 0;
    }
    ...
    /**
     * Get the total number of bits needed to writeout the stream used to create
     * this table.
     *
    ...
    private void calcStreamBits(List<HuffmanNode> nodes) {
        totalBits = 0;
    ...
    void addEntry(HuffmanNode node) {
        HuffmanNode found = values.get(node);
    ...
    public void writeDict(DataOutputStream dos) throws IOException {
        List<HuffmanNode> nodeList = new LinkedList<HuffmanNode>();
...
//System.out.println("packer bits: " + (dataLength * size + maxTagLength * size + size));
        ...
        // Write values
        Iterator<HuffmanNode> i = nodeList.iterator();
        HuffmanNode n;
...
//System.out.println("Dict entries: " + values.size() + " dataLength: " + dataLength + " DictSize: " + (dos.size() - streamPos));
    ...
    /**
     * Read the dictIonary from a stream.
     *
...
//System.out.println("found tag: " + tag + " = " + node.getValue());
    ...
    //
    private void computeTags(List<HuffmanNode> nodes) {
        ...
        node0 = ((LinkedList<HuffmanNode>) nodes).removeFirst();
        while (nodes.size() > 0) {
            node1 = ((LinkedList<HuffmanNode>) nodes).removeFirst();
    ...
    //
    private void addNodeInOrder(List<HuffmanNode> l, HuffmanNode node, Comparator<HuffmanNode> c) {
        ListIterator<HuffmanNode> i = l.listIterator(0);

        while (i.hasNext()) {
            HuffmanNode n = i.next();
     ...
     */
    void print(String header, Collection<HuffmanNode> nodes) {
        System.out.println(header + "\nentries: " + nodes.size() + "\n");

        Collections.sort((LinkedList<HuffmanNode>) nodes, IntegerHuffmanNode.dataComparator);
        for (HuffmanNode n : nodes) {
    ...
    /**
     * Reads a stream and converts the next 4 entries into an integer. Provide a
     * preallocated 4 byte buffer for buff.
     */
